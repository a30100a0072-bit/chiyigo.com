...
;(function () {
  ...
  function readDeviceUuidForRefresh(): string | null {
  ...
  async function doRawRefresh(): Promise<boolean> {
    ...
    try {
      ...
      if (!devId) return false   // 缺既有 device id → fail-closed，不打 refresh
      const hdrs: Record<string, string> = { 'Content-Type': 'application/json', 'X-Device-Id': devId }
      ...
      // 或覆蓋較新的登入 token。
      if (authEpoch !== startEpoch) return false
    ...
    if ('locks' in navigator) {
      try {
        await navigator.locks.request(LOCK_NAME, { mode: 'exclusive' }, async function () {
          ...
          const lockBudget = new Promise<void>(function (resolve) {
