...
;(function () {
  ...
  async function _doFetch(url: string, init: ApiFetchInit): Promise<Response> {
  ...
  async function apiFetch<T = unknown>(input: string, init: ApiFetchInit = {}): Promise<T> {
    const url = input
    ...
    if (!res.ok) {
      const bodyShape = (body && typeof body === 'object' ? body : null) as ApiErrorBodyShape | null
      throw new ApiError({
        status:  res.status,
        traceId,
  ...
  const API_ERROR_I18N: Record<string, Record<string, string>> = {
    'zh-TW': {
      INVALID_OTP:               '驗證碼錯誤',
      ...
      FACTOR_ADD_GRANT_REQUIRED: '需先完成身分驗證才能新增登入方式',
      FACTOR_ADD_ELEVATION_REQUIRED: '驗證已失效或過期，請重新操作',
      FACTOR_ADD_GRANT_CONSUMED: '驗證已使用，請重新操作',
      ELEVATION_SID_REQUIRED: '請重新登入後再試',
      ELEVATION_REQUIRES_2FA: '請先啟用兩步驟驗證',
      ELEVATION_USE_TOTP: '此帳號已啟用兩步驟驗證，請改用驗證碼',
      ELEVATION_NO_PASSWORD: '此帳號尚未設定登入密碼',
      EXCHANGE_CODE_INVALID: '驗證連結已失效或已使用，請重新操作',
      EXCHANGE_CODE_REQUIRED: '缺少驗證碼，請重新操作',
      ELEVATION_PROVIDER_NOT_BOUND: '此登入方式尚未綁定，無法用於重新驗證',
    ...
    en: {
      ...
      FACTOR_ADD_GRANT_REQUIRED: 'Please verify your identity first to add a login method',
      FACTOR_ADD_ELEVATION_REQUIRED: 'Verification expired or invalid — please try again',
      FACTOR_ADD_GRANT_CONSUMED: 'Verification already used — please try again',
      ELEVATION_SID_REQUIRED: 'Please log in again and retry',
      ELEVATION_REQUIRES_2FA: 'Please enable 2FA first',
      ELEVATION_USE_TOTP: 'This account has 2FA — use your verification code instead',
      ELEVATION_NO_PASSWORD: 'This account has no password set',
      EXCHANGE_CODE_INVALID: 'This verification link has expired or was already used — please try again',
      EXCHANGE_CODE_REQUIRED: 'Missing verification code — please try again',
      ELEVATION_PROVIDER_NOT_BOUND: 'This login method is not linked, so it cannot be used to re-verify',
    ...
    ja: {
      ...
      FACTOR_ADD_GRANT_REQUIRED: 'ログイン方法を追加するには、先に本人確認が必要です',
      FACTOR_ADD_ELEVATION_REQUIRED: '認証の有効期限が切れたか無効です。もう一度お試しください',
      FACTOR_ADD_GRANT_CONSUMED: '認証は既に使用されました。もう一度お試しください',
      ELEVATION_SID_REQUIRED: '再度ログインしてからお試しください',
      ELEVATION_REQUIRES_2FA: '先に2FAを有効にしてください',
      ELEVATION_USE_TOTP: 'このアカウントは2FAが有効です。認証コードをご利用ください',
      ELEVATION_NO_PASSWORD: 'このアカウントにはパスワードが設定されていません',
      EXCHANGE_CODE_INVALID: '確認リンクの有効期限が切れたか、既に使用されています。もう一度お試しください',
      EXCHANGE_CODE_REQUIRED: '確認コードがありません。もう一度お試しください',
      ELEVATION_PROVIDER_NOT_BOUND: 'このログイン方法は連携されていないため、再認証に使用できません',
    ...
    ko: {
      ...
      FACTOR_ADD_GRANT_REQUIRED: '로그인 방법을 추가하려면 먼저 본인 확인이 필요합니다',
      FACTOR_ADD_ELEVATION_REQUIRED: '인증이 만료되었거나 유효하지 않습니다. 다시 시도해 주세요',
      FACTOR_ADD_GRANT_CONSUMED: '인증이 이미 사용되었습니다. 다시 시도해 주세요',
      ELEVATION_SID_REQUIRED: '다시 로그인한 후 시도해 주세요',
      ELEVATION_REQUIRES_2FA: '먼저 2FA를 활성화해 주세요',
      ELEVATION_USE_TOTP: '이 계정은 2FA가 활성화되어 있습니다. 인증 코드를 사용하세요',
      ELEVATION_NO_PASSWORD: '이 계정에는 비밀번호가 설정되어 있지 않습니다',
      EXCHANGE_CODE_INVALID: '확인 링크가 만료되었거나 이미 사용되었습니다. 다시 시도해 주세요',
      EXCHANGE_CODE_REQUIRED: '확인 코드가 없습니다. 다시 시도해 주세요',
      ELEVATION_PROVIDER_NOT_BOUND: '이 로그인 방법은 연결되어 있지 않아 재인증에 사용할 수 없습니다',
  ...
  const BACKEND_ERR_LEGACY_MAP: Record<string, string> = {
  ...
  function tApiError(e: unknown, fallback?: string): string {
    if (!(e instanceof ApiError) || e.status === 0) return fallback ?? ''
    const dict = API_ERROR_I18N[_getLang()] || API_ERROR_I18N['zh-TW']
    const bodyShape = (e.body && typeof e.body === 'object' ? e.body : null) as ApiErrorBodyShape | null
    const code = e.code || (bodyShape?.error ? BACKEND_ERR_LEGACY_MAP[bodyShape.error] : null) || null
    let base: string = (code && dict[code]) || e.message || fallback || ''
    if (code && dict[code]) {
      base = base.replace(/\{(\w+)\}/g, (_, k: string) => {
        const v = bodyShape?.[k]
  ...
  function formatApiError(e: unknown, fallback: string = 'Something went wrong'): string {
