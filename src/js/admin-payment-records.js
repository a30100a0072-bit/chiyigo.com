...
async function load() {
  ...
  const r = await fetch(`/api/admin/payments/intents?${buildQs(currentPage, 50)}`, {
...
async function exportCsv() {
  ...
  try {
    ...
    while (page <= 500 /* hard cap */) {
      const r = await fetch(`/api/admin/payments/intents?${buildQs(page, 200)}`, { headers: { Authorization:`Bearer ${tok}` } });
      if (!r.ok) break;
