...
CREATE TABLE refresh_tokens (
  ...
  expires_at  TEXT    NOT NULL,
  revoked_at  TEXT
, auth_time TEXT, scope TEXT, issued_aud TEXT);
...
CREATE TABLE audit_log (
  ...
  ip_hash     TEXT,
  event_data  TEXT,
  created_at  TEXT    NOT NULL DEFAULT (datetime('now')), archived_at TEXT, cold_class TEXT NOT NULL DEFAULT 'immutable',
...
CREATE TABLE audit_archive_chunks (
  ...
  created_at         TEXT    NOT NULL DEFAULT (datetime('now')),
  updated_at         TEXT    NOT NULL DEFAULT (datetime('now')), dry_run INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX uq_rrr_intent_pending
  ON requisition_refund_request(intent_id)
  WHERE status = 'pending';
CREATE INDEX idx_used_totp_used_at ON used_totp(used_at);
CREATE INDEX idx_refresh_tokens_issued_aud ON refresh_tokens(issued_aud);
CREATE INDEX idx_audit_log_archived_at ON audit_log(archived_at);
CREATE INDEX idx_audit_log_cold_id    ON audit_log(cold_class, id);
...
CREATE UNIQUE INDEX uniq_agg_tele_bucket ON audit_log_aggregate_telemetry(
