...
const { results, warnings, withCode = [] } = data;

// ---- Parse public/js/api.js#API_ERROR_I18N to get 4-lang dict coverage ----
// 不 import（IIFE 不易 require），用 regex 切出每個語言 block 的 keys。
function parseApiDict() {
  const src = readFileSync('public/js/api.js', 'utf8');
  const langs = ['zh-TW', 'en', 'ja', 'ko'];
  const dict = {};
  for (const lang of langs) {
    // 找 `'lang': {` 或 `lang: {`（bare identifier）的 block，配對 `}` 結束
    const escaped = lang.replace(/[-\\^$*+?.()|[\]{}]/g, '\\$&');
    const re = new RegExp(`(?:['"\`]${escaped}['"\`]|\\b${escaped}\\b)\\s*:\\s*\\{`);
    const m = re.exec(src);
    if (!m) { dict[lang] = new Set(); continue; }
    let i = m.index + m[0].length, depth = 1, inStr = null, escape = false;
    ...
    const keys = new Set();
    const reKey = /^\s*([A-Z][A-Z0-9_]*)\s*:/gm;
    let km;
    while ((km = reKey.exec(body)) !== null) keys.add(km[1]);
    dict[lang] = keys;
  }
  return dict;
}
const apiDict = parseApiDict();
const dictUnion = new Set([...apiDict['zh-TW'], ...apiDict.en, ...apiDict.ja, ...apiDict.ko]);

// 已有 code 但 dict 缺翻譯（任一 lang 缺都算）
const codeToSites = new Map();
for (const w of withCode) {
  if (!codeToSites.has(w.code)) codeToSites.set(w.code, []);
  codeToSites.get(w.code).push(`${w.file}:L${w.line}`);
}
const missingDict = [];
for (const [code, sites] of codeToSites) {
  const missLangs = ['zh-TW','en','ja','ko'].filter(l => !apiDict[l].has(code));
  if (missLangs.length > 0) {
    missingDict.push({ code, missLangs, sites: sites.slice(0, 5), total: sites.length });
  }
}
missingDict.sort((a, b) => b.total - a.total || a.code.localeCompare(b.code));
...
const MANUAL_CODE_MAP = {
  ...
  // 命名選擇：用 TFA_* 而非 2FA_*（避免 leading digit identifier，且 dict 沿用 TFA_*）
  '2FA is already enabled': 'TFA_ALREADY_ENABLED',
  '2FA is not enabled': 'TFA_NOT_ENABLED',
...
out.push(`- 已有 code 但前端 dict 缺翻譯：**${missingDict.length} 個 code**（列在「漏譯」區）`);
out.push('');
out.push('### 判定規則');
...
if (missingDict.length) {
  out.push('## 漏譯區（已有 code 但 public/js/api.js#API_ERROR_I18N dict 缺鍵）');
  out.push('');
  out.push('> 後端已附 code，但前端字典任一語言缺對應 key → `tApiError` fallback 回英文 `error` 字串。補上即多 1 個 code 走 i18n 路徑。');
  out.push('');
  out.push('| code | 缺 lang | 出現處數 | 範例 site |');
  out.push('|---|---|---:|---|');
  for (const md of missingDict) {
    const miss = md.missLangs.join('/');
    const example = md.sites[0] + (md.total > md.sites.length ? ` …+${md.total - md.sites.length}` : '');
    out.push(`| \`${md.code}\` | ${miss} | ${md.total} | ${example} |`);
  }
  out.push('');
}
