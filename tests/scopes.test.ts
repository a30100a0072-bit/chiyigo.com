...
describe('PR2 — admin:billing scope + elevated:billing', () => {
  it('admin / developer / super_admin → 含 admin:billing coarse + 展開 :grant', () => {
    for (const role of ['admin', 'developer', 'super_admin']) {
      expect(scopesForRole(role)).toContain(SCOPES.ADMIN_BILLING)
      const eff = effectiveScopesFromJwt({ role })
      expect(eff.has(SCOPES.ADMIN_BILLING)).toBe(true)
      expect(eff.has(SCOPES.ADMIN_BILLING_GRANT)).toBe(true)
    }
  })

  it('coarse admin:billing token → 自動展開 admin:billing:grant', () => {
    const eff = effectiveScopesFromJwt({ scope: 'admin:billing', role: 'player' })
    expect(eff.has(SCOPES.ADMIN_BILLING_GRANT)).toBe(true)
  })

  it('只給 admin:billing:grant fine → 不會長出 coarse（最少特權）', () => {
    const eff = effectiveScopesFromJwt({ scope: 'admin:billing:grant', role: 'player' })
    expect(eff.has(SCOPES.ADMIN_BILLING_GRANT)).toBe(true)
    expect(eff.has(SCOPES.ADMIN_BILLING)).toBe(false)
  })

  it('admin:billing coarse 不誤展開到 admin:payments（兩 namespace 獨立）', () => {
    const eff = effectiveScopesFromJwt({ scope: 'admin:billing', role: 'player' })
    expect(eff.has(SCOPES.ADMIN_PAYMENTS)).toBe(false)
    expect(eff.has(SCOPES.ADMIN_PAYMENTS_REFUND)).toBe(false)
  })

  // OWNER DECISION（plan §15，未決）：finance 是否得 admin:billing:grant。目前暫不給，
  // 此測試鎖定「現況=finance/support 無 admin:billing」；owner 拍板給 finance 時改此測 + role map。
  it('finance / support 目前不得有 admin:billing（finance 為 owner 未決，暫不給）', () => {
    for (const role of ['finance', 'support']) {
      const eff = effectiveScopesFromJwt({ role })
      expect(eff.has(SCOPES.ADMIN_BILLING)).toBe(false)
      expect(eff.has(SCOPES.ADMIN_BILLING_GRANT)).toBe(false)
    }
  })

  it('elevated:billing 為 elevated scope（regular-token guard 會拒）；絕不出現在任何 role base', () => {
    expect(isElevatedScope(SCOPES.ELEVATED_BILLING)).toBe(true)
    expect(KNOWN_ELEVATED_SCOPES.has(SCOPES.ELEVATED_BILLING)).toBe(true)
    for (const role of ['admin', 'developer', 'super_admin', 'finance', 'support', 'player', 'user']) {
      expect(scopesForRole(role)).not.toContain(SCOPES.ELEVATED_BILLING)
    }
  })
})
