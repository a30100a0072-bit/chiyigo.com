...
const TASKS = [
  ...
  // audit_log：F-3 Phase 2（codex round-3 H-1，2026-05-10）
  //   原 audit_log_archive special 路徑已 neuter；Phase 2 archive worker 接手。
  //   詳見 docs/AUDIT_RETENTION_PLAN.md PR 0 Step 0.1。
  //   ⚠️ 不要重新加回此 task，否則 AUDIT_ARCHIVE_BUCKET binding 一加就觸發舊邏輯
  //      （無 manifest / 無 sha256 verify / 無 archived_at / 無 chunk state）。
...
// audit_log archive 流程移交 F-3 Phase 2 archive worker。
// 舊 archiveAndDeleteAuditLog() 已於 PR 0 Step 0.1 刪除（codex round-3 H-1 修正）：
// 舊路徑無 manifest、無 sha256 verify、無 archived_at marker、無 chunk state machine、
// 無 retry counter，跟 v6 設計（docs/AUDIT_RETENTION_PLAN.md）不相容；保留會導致
// AUDIT_ARCHIVE_BUCKET binding 一加就走錯路。Phase 2 worker 落地後從另一個檔案接手。
