...
- 整合測試（兩檔皆 direct-import 本檔 handler，grep 證實）：
  ...
  - `tests/integration/member-endpoints.test.ts` — `import { onRequestPost as createTenant }`（POST create→201·`org.created` / same-key replay→200·audited-once / diff-payload→409 `IDEMPOTENCY_CONFLICT` / unknown-field→400 / per-user rate-limit→429 runtime 驗證）。**註**：`contention`(503) / `invalid` / `default`(500) 分支**無 endpoint 測例**；本 PR type-only TS-erase 不碰這些分支，code-gate 報告**不宣稱** contention 已實跑（Codex plan-gate Low finding）。
