...
- **授權邊界進程**：owner ① R1 期核准僅 plan doc（未授權 env.d.ts/CODING）→ ①②✅ 後 owner **明示 `CODING_ALLOWED`** → CODE source（env.d.ts +10 key + 本 receipt 更新）本 commit。**仍未授權**：push / PR / merge / deploy（待 ③④ + owner `MERGE_ALLOWED` + CI green）。
