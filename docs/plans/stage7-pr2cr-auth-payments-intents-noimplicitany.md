...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ✅ `PLAN_DRAFT` — 本 doc。
  - ⬜ `PLAN_SELF_REVIEW_CLEAN`（multi-agent workflow；三維 rubric scope / runtime·security / evidence；readonly-reviewer 繼承 Opus 4.8；主線獨立裁決、非採 raw — 跑完補 §Gate 進程紀錄）
  - ⬜ `CHATGPT_ARCH_APPROVED`（① 維度 B）→ ⬜ `CODEX_PLAN_APPROVED`（② 維度 C）→ ⬜ owner `CODING_ALLOWED`
  - ⬜ Code 階段（source commit、full replay @ committed、NB-2 雙證）→ ⬜ `CODE_SELF_REVIEW_CLEAN`（維度 A workflow）→ ⬜ `CODEX_CODE_APPROVED`（③ 維度 C）→ ⬜ `CHATGPT_CODE_FAITHFULNESS_APPROVED`（④ 維度 B-code）
...
- ⬜（待跑）multi-agent workflow self-review（維度 A）→ `PLAN_SELF_REVIEW_CLEAN`。
- ⬜（後續 dated 收錄：plan doc commit → 中文報告 → ChatGPT Arch packet → ① → Codex Plan packet → ② → owner CODING_ALLOWED → Code 階段 → ③ → ④ → merge-front 7 gates → MERGE_ALLOWED → squash → SHIPPED）
