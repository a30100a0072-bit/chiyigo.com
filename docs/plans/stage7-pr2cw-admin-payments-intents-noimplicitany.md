...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ✅ `CHATGPT_ARCH_APPROVED_WITH_LOCKS`（① 維度 B、2026-06-28、**0 blocker / 0 required revision**、6 審查點全 APPROVED/APPROVED_WITH_LOCK、新增 binding **BL-10 MainRow non-precedent lock**；downstream 保留 BL-1..BL-10；見 §Gate 進程紀錄）→ ⏳ `CODEX_PLAN_APPROVED`（② 維度 C、**待送**）→ ⏳ owner `CODING_ALLOWED`（**待明示**）
...
- ⏳（後續 dated 收錄：Codex Plan packet → 外部 ② → owner `CODING_ALLOWED`）
