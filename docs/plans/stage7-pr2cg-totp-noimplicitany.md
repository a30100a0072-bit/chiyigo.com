...
- **下一步**：→ **ChatGPT faithfulness**（plan-faithful / 方向對齊）→ owner 明示同意 → squash-merge（`--delete-branch`）→ merge 前跑齊 CI 對應 local gates（含 `test:cov` / `npm audit` / `verify:browser-pipeline`，[[feedback_pre_merge_gate_checklist_match_ci]]）→ 監看 main CI + Deploy。
