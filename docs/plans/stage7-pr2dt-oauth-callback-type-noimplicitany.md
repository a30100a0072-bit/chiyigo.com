...
**性質**: 純 type-only noImplicitAny 標註（**23→0 於 callback.ts 的 27 個中；exchangeCode 的 4 個 TS7031 明示保留、descope 到棒5**）、byte-identical emit（esbuild stdin-pipe + SHA-256 + cmp -s 實證）、**零 runtime AST / 零 control-flow / 零 validation / 零 schema / 零 runtime HTTP·wire·行為契約 / 零 migration / 零部署面 / 零 test 改動 / 零 env.d.ts / 零新 interface / 零 export / 零新套件 / 零 lockfile**（⚠ exported handler 的 compile-time callable signature 有收窄 hardening、非 runtime 契約變動、見 §1.2.1 RR1）。棒4-type 是 reduce PR（**REMOVED=23**）；post-ratchet = **385 / 18 / 317**（callback 仍 dirty、留 4 錯）。
...
> **type surface（zero 新 named interface、zero export、zero 新 import）**：handler-ctx inline 標註 ×3（1 個 shape：init 已用 `[key:string]:unknown` envelope、precedent 見 §2）+ `let` JWKS 型 **2 處標註**（各解消 3 error = 6 error，對映 SPEC-C-2『JWKS `let` ×6』的 error 口徑）+ function param 標註（fetchProfile 4 / verify 8 / escapeHtml 1 / htmlError 1）；`ReturnType<typeof getProvider>` 與 `ReturnType<typeof createRemoteJWKSet>` 皆槓桿既有 import（`getProvider` L23、`createRemoteJWKSet` L19）、**零新 import**；`Env`/`Request` global ambient（callback.ts 未 import，overlay ADDED=0 反證解析）。callback.ts = leaf route handler、**不新增 named/exported type** → **helper 型別面 module-local**；但 `onRequestGet`/`onRequestPost` 的 **exported callable signature 由 implicit `any` 收窄（compile-time contract hardening、§1.2.1 RR1）**（production 僅 Pages router 觸發；integration test 以 value import `onRequestGet`，故 CASCADE 含 tests-leaf，見 §6 NB-1）。
...
>
> **① ChatGPT Arch 鎖定 downstream 權威說法**（gate packet／receipt／hunk 驗收禁把『23』描述為 syntactic annotation site）：
> ```
> 23 noImplicitAny diagnostics removed
> via 19 explicit type-annotation sites
> across 11 changed source lines
> ```
> SPEC-C-2 owner lock 原文『23 標註』保留不改（其精確語意 = 23 diagnostics、見 (1)）。
...
#### 1.2.1 SPEC-C-4「禁 runtime AST 變更」的**聯合證明**（① ChatGPT Arch RR2 修正）
SPEC-C-4「零 runtime AST 變更」由**兩項核心證據聯合坐實**——**byte equality 不單獨宣稱 source AST identity**（不同 source AST 可經型別擦除/轉換產生相同 JavaScript；byte equality 單獨只證「固定 esbuild 版本/loader/format/輸入下，base 與 candidate 的 **emitted JavaScript bytes** 相同」）：
1. **完整 source hunk allowlist（§4 PROVIDER-PATH-HUNK）**：`git diff 67111ad4..<source>` 證明唯一 source delta = 允許的 TypeScript 型別標註（參數 `: Type` + `let` 型），**無** guard/branch/call/import·export/rename/reorder/error-handling。
2. **pinned-base byte-identical replay（§3.B）**：`git show 67111ad4:<f> | esbuild` vs candidate 兩面，證明這些 source delta **未改變 emitted JavaScript bytes**（型別標註於 esbuild `--loader=ts` emit 全 strip）。
→ (1)+(2) **聯合** ⟹ SPEC-C-4「零 runtime AST 變更」（(1) 界定 source delta 僅型別標註、(2) 證其 emit 不變）。**另**：§3.A `REMOVED=23 / ADDED=0` 證型別目標達成且無 compiler cascade（佐證 source delta 確為 type-only、非 SPEC-C-4 之 AST-invariance 直接證據）。

**⚠ compile-time contract 揭露（① ChatGPT Arch RR1）**：本棒**不改 runtime HTTP／wire／schema／status／cookie／行為契約**；但 `onRequestGet`、`onRequestPost` 的 **exported TypeScript callable signature** 由 implicit `any` 收窄為明確 inline context shape，屬 **compile-time contract hardening**（exported value signature 是模組契約一部分、即使不新增 named/exported type）。全 solution set-diff `ADDED=0` 證明**目前已知 in-repo consumers 與新簽名相容**（非「契約未變」）；不新增 named/exported type。
...
**主線 fresh-pass re-read（修正後）**：grep 全文確認無殘留 `git show HEAD:` 當 recipe（僅 1 次為「禁用」警告）、無 ×2/×6 互斥、before 兩處鎖 67111ad4、23/19/11 三計數並存且各有機械來源。**修正皆 plan-doc 層（不觸架構/scope/型別決策/REMOVED=23）→ 依 Dual Gate §9 回路節流用主線對抗式 re-read、未重跑 7-finder workflow**。**一輪 0 新發現 → `PLAN_SELF_REVIEW_CLEAN`**（@ `e14e8bfd`）。

### 5.6 ① ChatGPT Architecture Gate R1（`CHANGES_REQUESTED @ e14e8bfd`、2026-07-13）
架構方向 **PASS**、Path C 拆分 **PASS**；**2 Tier-1 Required、0 scope-reversal、0 security-blocker、無 source 授權**。已通過項：Path C/SPEC-C-7 忠實/env:Env vs ProviderSecretsEnv/ReturnType 複用/JWKS ReturnType/index-sig/migration/fresh replay/棒4-5 隔離。2 條 Required 皆**契約與證據措辭校正**（非 redesign、非 scope 變）：
- **RR1（Tier-1）exported handler compile-time contract delta**：`onRequestGet`/`onRequestPost` exported callable signature 由 implicit `any` 收窄 = **模組契約一部分**（即使不新增 named/exported type）；`ADDED=0` 只證 in-repo consumers 相容、**非契約未變**。→ **修**：§8 性質「零 API」→「零 runtime HTTP·wire·行為契約」+ §1.2.1 加 compile-time hardening 揭露 + §1.1 type-surface「型別面全 module-local」→「helper module-local；exported signature hardening」+ NB-1「無 public type contract」→「不新增 named type、但 exported value signature 收窄」。
- **RR2（Tier-1）byte-identical ≠ source AST identity 雙向證明**：`emit 逐 byte 相同 ⟺ runtime AST 零變更` 過強（不同 AST 可擦除成相同 JS）。→ **修 §1.2.1**：改**聯合證明**〔完整 hunk allowlist（source delta 僅型別標註）+ pinned-base byte replay（emit 不變）+ REMOVED=23/ADDED=0（無 cascade）〕；去掉 `⟺` 單獨等價；「JS bytecode」→「emitted JavaScript bytes」。byte replay recipe 本身 ChatGPT 判 sound、未改。
- **計數校準**：ChatGPT 鎖 downstream 權威說法「23 diagnostics / 19 annotation sites / 11 changed lines」（§1.1 已加）；SPEC-C-2 owner lock 原文保留。

**主線對抗式 re-read（RR1/RR2 修正後、對 ChatGPT grep 清單 `zero API`/`零 API`/`public type contract`/`module-local`/`bytecode`/`runtime AST`/`⟺`/`23 標註`）**：
- `bytecode`：已從 **proof claim 全清**（§1.2.1 用「emitted JavaScript bytes」）；殘 3 處僅本 §5.6 / §receipt **記錄修正動作**的 meta-引用（「JS bytecode→emitted JavaScript bytes」），非錯誤 claim。
- `⟺`：**雙向等價已從 §1.2.1 proof 移除**（改單向 `⟹` + 兩支柱聯合）；`⟺` 符號僅殘本 §5.6 **引用舊 claim** 之修正記錄。
- 「零 API」→「零 runtime HTTP·wire·行為契約」；`module-local` → 皆加 exported-signature-hardening 但書（§1.1/§1.2.1/NB-1）；`public type contract` 皆帶「非零、exported value signature 收窄」但書；「23 標註」僅存 SPEC-C-2 owner verbatim + 明確 gloss；`runtime AST` 皆為聯合證明之**結論**（非單獨 byte-equality 雙向宣稱）。

**修正皆 docs-only（不改 scope/型別決策/SPEC-C-1..10 引用/REMOVED=23/ratchet/byte recipe/source）→ §9 主線 re-read、未重跑 workflow。一輪 0 新發現 → `PLAN_SELF_REVIEW_CLEAN_R2`**、待 ① 窄幅 re-review。

## 6. 非 blocking notes
- **NB-1（① ChatGPT Arch RR1 修正）**：callback.ts = leaf route handler、**不新增 named/exported type**——production 僅 Pages router 觸發。⚠ **但 exported `onRequestGet`/`onRequestPost` 的 callable signature 由 implicit `any` 收窄為明確 context shape（compile-time contract hardening、§1.2.1）——非「零 public type contract」**；exported value signature 是模組契約一部分。integration test 以 **value** import：`callback.test.ts`〔`onRequestGet as cbGet` 靜態〕、`oauth-nonce.test.ts`〔`onRequestGet as cbGet` 靜態 + `init` 動態 import〕。故 CASCADE（SPEC-C 對映 ARCH-3B-12）**必含 tests-leaf**；overlay `ADDED=0` 證**目前已知 in-repo consumers 與新簽名相容**（非契約未變）。
...
- SPEC-C-1..10（allowlist 僅 callback / 23 diagnostics〔19 標註 site·11 line〕 / exchangeCode 保留 / 禁 runtime AST〔hunk allowlist + byte replay 聯合證明〕 / ratchet 385-18-317 halt-drift / byte-equality 主判準 / 禁宣稱全清 / 棒5 子 scope / 棒5 重走 gate / 禁交錯）。
- ① ChatGPT Arch R1 `CHANGES_REQUESTED @ e14e8bfd`：RR1（exported handler callable signature = compile-time contract hardening、非「零 API/public type contract」）+ RR2（byte equality 不單獨 = source AST identity → hunk allowlist + byte replay + ADDED=0 聯合證明；bytecode→emitted JavaScript bytes）。皆 docs-only 窄修、新 commit（非 amend）。
