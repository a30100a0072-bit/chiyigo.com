...
> ⚠ 「唯一」**限定於 classifier boundary**，非字面「全 repo 唯一」——
...
| 項 | 規定 |
|---|---|
| **唯一性（範圍已限定）** | **classifier boundary 上恰一個** canonical parser／guard，涵蓋 §5 表列之**四條 production classifier call path**。後續棒次一律 **import 重用**，🚫 禁在該 boundary 上各自重寫 inline guard |
...
| 必定案項 | 約束 |
|---|---|
| parser／guard API | **單一 canonical implementation**；**classifier boundary 上**其他 caller **只能 import**（範圍同上表之限定，🚫 不得回到「全 repo 唯一」的過寬表述；`admin/audit.ts` 之 query validator 不在此列） |
