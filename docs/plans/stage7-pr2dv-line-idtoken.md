...
**Gate state**: ①`CHATGPT_ARCH_APPROVED`（RR1 CLOSED、2026-07-15）→ ②`CODEX_PLAN_APPROVED`（R3、2026-07-15）→ owner `CODING_ALLOWED`（2026-07-17）→ CODE stage 實作 + 機械證據完成 → ⚠ **base-RED 實測掀出 N15（F-2 null-secret）→ owner 裁「回 Plan Gate 正式修 matrix」** → `PLAN_AMENDMENT_R4` → ①`CHATGPT_ARCH_CHANGES_REQUESTED`（R4：Required 3）→ 落修 → **①`CHATGPT_ARCH_APPROVED`（R4 窄複審 + R4b comment-only、2026-07-17；核准矩陣 **DELTA 10 / INVARIANT 6**、base `18b5f72d`）** → **②`CODEX_PLAN_APPROVED`（R4-R5、2026-07-17）** ✅ → owner `CODING_ALLOWED`（2026-07-17）→ **CODE stage 完成**（commits `0abd39d8`→`5f3d99f6`→`38dfaf51`；DOC-SYNC-LOCK 已執行）→ **維度 A self-review `CODE_SELF_REVIEW_CLEAN`**（6 accepted 全處置 + self-review-of-self-review 撤回 overclaim）→ merge-front gates 全綠 → **現＝送 ③ Codex Code Gate**｜`MERGE/PUSH=NOT_ALLOWED`（待 ③④ + DEPLOY-EVIDENCE + owner squash 授權）｜**§13/§14 為本輪 SoT**
...
- **#5（tier2、accepted）N11 main 弱鎖**：`oauth-nonce.test.ts:202` 原僅斷言 `status===400`，但所有 fetchProfile 失敗都回 400（htmlError 預設）→ 無法歸因 nonce gate。**owner 2026-07-17 裁「就地強化」**：補 `toContain('id_token nonce mismatch')` 精確釘 gate + `users` count=0 零副作用。仍 DELTA（base 對此 fixture 回 200→訊息/副作用斷言仍 RED、矩陣不變）。
  - ⚠ **self-review-of-self-review 更正**：finding 另建議「fixture id_token 改帶合法 nonce 以隔離失敗模式」，我一度採納並如此宣稱——**但機械實測推翻**：verifier 兩道 nonce guard（stored-NULL check + `payload.nonce !== expectedNonce`）對 `expectedNonce=null` 為**冗餘**（token 帶任何 nonce 時 line 2 仍 `!== null` → throw），故**任何 fixture 都無法隔離「只刪其一」的 mutation**（實測：刪 line 1 + 帶 token nonce，8 tests 仍全 pass）。已**撤回 fixture 變更**、更正「隔離」宣稱為假。本強化的真實價值＝訊息歸因 + 零副作用，非 fixture 隔離。（教訓：又一次「assert without verify」，於自審之自審抓回。）
  - ⚠ **超出 §4.1/§5.1 字面 N11-NARROW-EDIT（「僅 expectation 200→400」）**：owner 明示授權、依 standing rule [[feedback_regression_test_must_lock_exact_failure]] + PR-2du F-3 先例（test 品質意圖 > 字面 scope）；④ faithfulness packet 標為 owner-approved self-review 強化。
