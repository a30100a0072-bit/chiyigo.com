...
**狀態**: `MERGE_ALLOWED`（**①②③④ 全 4 道 gate 過、皆 0 finding**：① `CHATGPT_ARCH_APPROVED_WITH_LOCKS` (r1) @ Arch anchor `a8e6fe42` · ② `CODEX_PLAN_APPROVED` (r3) @ plan-frozen `6718ccee` · ③ `CODEX_CODE_APPROVED` + ④ `CHATGPT_CODE_FAITHFULNESS_APPROVED` @ CODE `49e1ac0d` → owner `MERGE_ALLOWED`（2026-07-13）→ 執行 merge 序列：merge-front CI local gates → push → PR → CI green → squash-merge 進 main）。上游歷史：SPEC `SPEC_APPROVED_WITH_LOCKS` + ① Arch r0〔cast 版、superseded〕+ ② Plan r1/r2 `CHANGES_REQUESTED`〔cast→overload、doc-hygiene〕+ 維度 A self-review clean（plan §5.7 / CODE 2-lens）。
> **gate 進程**: scout〔2 檔共 6 錯 forced tsc 確認、zero dual-leaf〕→ transient overlay 實測〔**REMOVED=6/ADDED=0** + byte-identical 升級證據（cmp -s + SHA-256）+ `git checkout --` 還原、ratchet 還原 414/20/315〕→ owner **`SPEC_APPROVED_WITH_LOCKS`**（OD-3b-1..4、micro、MC-1/MC-2、ARCH-3B-1..16）→ MC-1 params 驗證〔`{ provider?: string }` REMOVED=6/ADDED=0 無 cascade、採用〕→ 本 plan doc → 維度 A self-review clean → **① ChatGPT Arch r0 `APPROVED_WITH_LOCKS`**〔2 提交前修正 folded〕→ **plan-anchor commit `45935374`（cast 版、歷史）** → **② Codex Plan Gate r1 `CHANGES_REQUESTED`**〔OD-3b-3 cast 違 TS-BOUNDARY-002 → function overload〕→ **plan r1 修訂（overload、零 assertion）+ 焦點 self-review + anchor `a8e6fe42`（parent `45935374`）** → **① ChatGPT Arch r1 `APPROVED_WITH_LOCKS`**〔overload 正式取代 cast〕→ **doc-hygiene follow-up `ec95b379`** → **② Codex Plan r2 `CHANGES_REQUESTED`**〔技術全綠、2 doc-hygiene major：gate-state SoT 矛盾 + return-type 描述〕→ **r2 doc-hygiene 修正 `6718ccee`（parent `ec95b379`）** → **② Codex Plan r3 `APPROVED`（plan FROZEN @ `6718ccee`）** → **owner `CODING_ALLOWED`（2026-07-13）** → **CODE source commit `49e1ac0d`（`init.ts` + `bind-email.ts` + header gate-state）** → **CODE fresh-replay gates 全綠**〔forced set-diff `414→408`／`REMOVED=6 ADDED=0`（含 tests leaf；**負控制**：拿掉 index-sig → 精確 7×TS2353 於 4 個 integration 檔，坐實 ADDED=0 非 vacuous）· byte-identical 雙面（working-tree + committed-blob）`init 9048/f87d1c0f…`・`bind-email 8060/f3200846…`、`cmp -s`=0 · ratchet `408/18/317`（baseline `1119/175` frozen）· 8 個 CI 對應 local gate 全 exit 0〕→ **CODE 維度 A self-review 2-lens（5 agents、3 candidate 全 REFUTED、accepted=0）** → **③ `CODEX_CODE_APPROVED` @ `49e1ac0d`**〔0 blocker/major/minor；獨立重現全部數字，含 errorFiles `20→18`〕→ **④ `CHATGPT_CODE_FAITHFULNESS_APPROVED` @ `49e1ac0d`**〔0 blocking / 0 scope deviation / 0 plan-fidelity deviation；ARCH-3B-1..16 逐條 PASS〕→ **owner `MERGE_ALLOWED`（2026-07-13）** → merge-front CI local gates → push → PR → CI green → squash-merge → main。
> **狀態 SoT**: 本 header + 對應中文報告為當前 gate-state 權威。目前＝**`MERGE_ALLOWED`**；**①②③④ 全 4 道 gate 過**（@ CODE `49e1ac0d40616f56bac2602d740690dba8e53541`、plan-frozen `6718cceeb7dfd7c52fcc69ea623b821c309299d9`、Arch `a8e6fe42c9f982314398673beab32beb410bb8ea`）、owner `MERGE_ALLOWED` 已授；執行 merge 序列：merge-front CI local gates → push → `gh pr create` → CI green → squash-merge → main。**禁直推 main / 禁 `--admin` 繞 review / 禁 force push**。⚠ **③④ 的 review anchor 固定為 source commit `49e1ac0d`**；本檔自 `6718ccee` 起僅 header gate-state 行 + §1.1 前 tree-note 變動（CODE commit 一次、本 merge-front commit 一次，**皆 docs-only**），**契約 `## 1.`→EOF 逐 byte 未改**（③ 獨立核：33,042 bytes、SHA-256 兩側相同）。歷史 anchor：`ec95b379e6085323b7828f7587194eed9f7d51a1`（Plan r2）、`45935374`（cast 版、superseded、**不可直接取得 Plan approval**）。
...
**owner ruling（2026-07-11 `SPEC_APPROVED_WITH_LOCKS`）**: source 核准僅 `init.ts` + `bind-email.ts`（+ plan companion）｜OD-3b-1 context shape = **inline object + `[key: string]: unknown`、禁共用 interface/export/正式 EventContext 宣稱**｜OD-3b-2 = **handler boundary `env: Env`、util 端 `ProviderSecretsEnv` 不放寬**｜OD-3b-3 = **~~`new Uint8Array(buf as ArrayBufferLike)`~~ → r1 修訂為 `toBase64Url` function overload〔零 assertion、閉合 advisory TS-BOUNDARY-002〕、① Arch r1 `APPROVED_WITH_LOCKS`**｜OD-3b-4 = **bind-email 不含 params**｜micro（`n: number` / `platform: string` / `port: string \| null`）全採｜**強制修正 MC-1（params 不得寬泛 Record）+ MC-2（byte-identical 證據升級）**｜Architecture Locks **ARCH-3B-1..16**（§1.2 逐字）｜風險表 §5.2 / 防禦表 §5.4 / 驗證要求 §5.5 / 商業摘要 §5.6。

> ⚠ 本棒 **tree 狀態（merge-front）**：唯一 untracked ＝ `CLEANUP_PLAN.md`（pre-existing、**不屬本 PR、挑檔 stage 排除**）。PR 的完整 changed-files allowlist 恰 {`init.ts`, `bind-email.ts`, 本 plan doc}——**source 面恰 {`init.ts`, `bind-email.ts`}，且僅於 CODE commit `49e1ac0d` 落地**；plan doc ＝ anchor 鏈 `45935374`→`a8e6fe42`→`ec95b379`→`6718ccee`（frozen）+ CODE commit 的 header gate-state + 本 merge-front header commit（後兩者皆 docs-only、契約區未動）。
...
> RUNTIME-LOCK（ARCH-3B-4/11）坐實：provider 驗證 / PKCE / state / nonce / silent SSO / rate-limit / is_binding factor-add / elevation OAuth-reauth / oauth_states 寫入 / temp_bind jti replay / reverification gate / 信箱碰撞 / user·identity 建立 / access·refresh token 簽發 / cookie 100% 未動；參數註記 / function overload 簽名（型別資訊）全於 emit 抹除〔esbuild strip overload signature、實測 init.ts emit 只剩 `function toBase64Url(buf) {`〕（**本棒零 `as` assertion；兩條 overload declaration 含核准的 `: string`、implementation signature/body 不新增 return-type annotation**，delta 限「參數型別 + overload 型別簽名」、ARCH-3B-3）。
