...
> **(Codex plan-gate advisory 1)** ledger 的 `leaf` / `flag` / `errorCount` / `cleanFiles` 以 ratchet 的 `[OVERRIDE]` 輸出行為 **source of truth**（由真實 tsconfig delta + baseline 推導），**不採** `workflow_dispatch` UI 的 `leaf` input（該 input 未被 workflow 機械比對 ratchet 偵測值，僅作觸發用、非權威）。ledger 直接貼 `[OVERRIDE]` 行原文。
