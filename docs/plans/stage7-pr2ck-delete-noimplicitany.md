...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ✅ `CODE_SELF_REVIEW_CLEAN`（multi-agent workflow，3 agents converged 三維：diff-fidelity / runtime·security / evidence；source `ed5a93af`）→ ✅ `CODEX_CODE_APPROVED`（維度 C，0 blocker / 0 critical risk）
  - ✅ `CHATGPT_CODE_FAITHFULNESS_APPROVED`（維度 B，11/11 faithful，0/0/0；**外部 4 道全通過**）→ ⬜ `MERGE_ALLOWED`（merge-front 7 gates 全綠 ✅、待 owner 明示）→ ⬜ `MERGED_MAIN`
...
- 2026-06-20 **process 自捕 + 修復（揭露，faithful 收錄）**：Code-stage 一個 docs-only gate-log commit `c8deef47` 因 Code self-review Explore agent（具 Bash）跑 stray `git checkout b5e76f69 -- delete.ts` 留下 base-staged index、被 `git add docs` 掃入，誤把 delete.ts annotation revert（該 commit 報「2 files changed」觸發警覺）。自捕後 `git reset --soft ed5a93af` + `git checkout ed5a93af -- delete.ts` 還原、doc-only re-commit `4bbb6db0`；`c8deef47` 已丟棄（非 HEAD ancestor、無 branch 包含）。修後再驗：net source diff = 唯二 2 annotation、byte-identical `541e4cfb…` IDENTICAL、ratchet OK 824/249。教訓：commit 前必核 `git diff --cached --name-status` + net source diff（self-review Explore agent 可改 git state）。
- 2026-06-20 **Codex Code Gate（③ 維度 C）：`CODEX_CODE_APPROVED`**（0 blocker / 0 critical risk / 0 required）— 機械核驗全通過並**獨立重驗**：HEAD `4bbb6db0`、5 commit 線性、`ed5a93af` 僅 delete.ts +2/−2、`4bbb6db0` 僅 gate-log、`c8deef47` 非 HEAD ancestor（index 污染完整消除、source/history 未受影響）、index 空、status 僅 `?? CLEANUP_PLAN.md`；forced solution 827→824 / REMOVED=3 / ADDED=0、ratchet 824/249（ceiling 1119/175）、tests-leaf 兩端 exit 0、emit 兩端 3721B sha `541e4cfb…9499`、lint green、Functions build successful；State Consistency **PASS**（逐行僅 L16/L31 annotation、其餘 SHA 一致、SQL/step-up/密碼/token/rollback/audit/回應未變）、Observability **PASS**（critical audit byte-identical、coverage 不 overclaim）、Queue/Payment（payments 隔離）/Distributed State **N/A**。**待 ④ ChatGPT Code Faithfulness；仍非 `MERGE_ALLOWED`。**
- 2026-06-20 **ChatGPT Code Faithfulness Gate（④ 維度 B）：`CHATGPT_CODE_FAITHFULNESS_APPROVED`**（Faithfulness Matrix **11/11 faithful**；0 blocker / 0 required / 0 NB）— actual committed source diff 與 approved frozen diff 完全一致（唯二 2 annotation、僅 `delete.ts`）；11 維 faithful：scope / edit-point / OD-ctx / worker annotation / error-target（TS7006 ctx + TS7031×2 removed）/ runtime-neutral（byte-identical 3721B sha 兩端一致）/ hot-zone lock / env lock / test lock / coverage honesty / gate locks。無 deviation；`c8deef47` index 污染非 faithfulness 問題（已丟棄、非 final history、net source = approved +2/−2）。**外部 4 道全通過**（① ChatGPT Arch + ② Codex Plan + ③ Codex Code + ④ ChatGPT Faithfulness）。
- 2026-06-20 **merge-front local gates 全綠（@ HEAD `706724a2`，CI-equivalent，[[feedback_pre_merge_gate_checklist_match_ci]]）**：`lint` ✅ · `typecheck:ratchet` ✅（`RATCHET_BASE_REF=b5e76f69`、current **824/249**、ceiling 1119/175 不變）· `verify:browser-pipeline` ✅（25 pages / 214 refs = committed content-hash、classic+module prod emit byte-equal）· `test:cov` ✅（Statements **90.28%** 1933/2141、Branches 92.77%、Functions 92.08%）· `test:int` ✅（**75 files / 1328 tests**）· `build:functions` ✅（Compiled Worker successfully）· `npm audit --omit=dev --audit-level=high` ✅（**0 vulnerabilities**）。post-gate status 僅 `?? CLEANUP_PLAN.md`。**待 owner `MERGE_ALLOWED` → squash-merge `--delete-branch`。**
- （後續 dated 收錄：MERGE_ALLOWED / squash-merge / main CI / Cloudflare deploy）
