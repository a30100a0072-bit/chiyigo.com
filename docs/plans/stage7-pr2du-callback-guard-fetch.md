...
```
`?: string`（Cloudflare env binding 一律 string）。**零 JS emit**、**無必要 DB/schema migration、無必要部署設定變更**——但**新增一個 optional runtime configuration contract**（Arch #Tier-2a 措辭修正：optional env key 即使 zero-emit 仍是新設定面，非「無部署面變更」）。**unset 語意（Arch R3 非阻塞註 1 精確化）**＝**無必要 env migration；unset 時啟用本 PR 新增的內建 bounded defaults（token=8000ms、userinfo=5000ms）**。⚠ **非**「與 pre-PR 現狀等價」——pre-PR 是**無界等待**（bare fetch 無 timeout），本 PR unset 後仍有 8s/5s 有界 timeout（這正是本 PR 目的）；「unset=現狀」只在「不需新增部署 binding」這層成立、runtime 行為已改為 bounded。先例 = PR-2dr 棒3-env #144（additive +10 optional key，Codex 正規化後 bundle SHA-256 相同＝zero-emit 鐵證）。
...
unset 語意                   : 啟用內建 bounded defaults token=8000ms / userinfo=5000ms（非 pre-PR 無界等待；Arch R3 註 1）
```
...
- 合計 **16 cases**（Codex Plan R2 加 T4b〔token body-stall〕+ R3 加 T8c〔userinfo rejection 耗盡〕）。**⚠ code-self-review #3 曾加 T17〔parseFetchTimeoutMs clamp/fallback unit〕，因需 production `export`〔逾越 approved module-local 設計〕經 ④ Faithfulness 判 scope creep → 已撤回**（clamp regression-lock 改 backlog、NB-14）。
...
- **兩類驗收實證（SPEC-D-5、Arch R2 + Codex Plan R2/R3）**：**DELTA_RED 10 條**（T1/T2/T4/T4b/T5/T5b/T8/T8b/T8c/T9）須留「在 base `274a37b4` 上真的 fail」證據；**INVARIANT_GREEN 6 條**（T3/T6/T6b/T7/T10/T11）須留「base GREEN ∧ candidate GREEN」證據。**CODE stage 已實測：base 10-fail/6-pass、candidate 16-pass。**
...
- **NB-14（parseFetchTimeoutMs clamp regression-lock — 撤回 export、改 backlog；④ Faithfulness F-1）**：code-self-review #3（tier2）指 `parseFetchTimeoutMs` 上限 clamp（15000）與 `<10`/invalid fallback 無 regression lock。曾為 T17 將其改 `export`，但 **④ ChatGPT Faithfulness 判此 export 逾越 approved plan 的 module-local 設計＝scope creep**（plan §4.3 刻意選 module-local；SPEC-D-6 不禁 ≠ 授權；CODE stage 無權反轉 plan 架構決策而不經 Plan Gate amendment）。→ **撤回 export + T17、回 16 cases module-local plan-faithful**（[[feedback_prefer_plan_fidelity_over_small_waiver]]）。clamp code 本身經 ③ Codex Code 覆核**正確**（僅未 unit-lock）。**backlog**：clamp/fallback 的 regression-lock 需 (a) 未來 Plan-amended PR 授權 export + unit test、或 (b) 慢速 behavioral integration test（設 override >15000、觀測 timeout 封頂 ~15s）——本 PR **不做**（低迴歸風險：clamp 為顯式 `Math.min(..., FETCH_TIMEOUT_MAX_MS)` named-const、移除屬可見刻意變更）。
> **教訓（記 memory）**：code-self-review finding 若要求**偏離 approved plan 的設計決策**（非單純收緊測試），正解＝回 Plan Gate amendment 或找 plan-faithful 替代，**不得**在 CODE stage 逕自實作再靠揭露補授權（④ 會擋、且應擋）。
- **NB-15（測試 helper 顯式標型；④ Faithfulness F-3、owner 裁決 accept）**：新測試檔的 file-local helper（`b64url`/`b64urlString`/`signLineIdToken`/`seedOauthState`/`stateRowExists`/`callGet`/`callPost`/`behave` + fetch mock `vi.fn`）**全部補真實邊界型別**（無 `any`）。④ 指此為 SPEC-D-4「新 code 每個 param 顯式標型」——雖 SPEC-D-4 字面 scoped 到 production（`否則 errorCount≠381`）+ 測試檔另有「clean」條款（已滿足），且 repo 既有 test 檔全未標型、OD-5-HELPER 明令複製 oauth-nonce.test.ts 形狀，**但治理意圖（防 any 回流、strict 遷移方向）較字面 scope 更重要**（owner 2026-07-15 裁 accept、④ 論點勝）。標型純編譯期/契約層、**零 runtime 變更**（guard/timeout/retry/env 皆未動）；forced-tsc 仍 381、ADDED=0、16/16、base 10-fail/6-pass。**教訓補充**：新測試檔即使在 `noImplicitAny:false` 的 tests leaf，仍應顯式標型 helper 參數（領先 legacy 未標型檔往 strict 走、不累積遷移債）。
