...
| **test** | `tests/integration/oauth-callback-guard-fetch.test.ts` | **新檔**：13 cases（DELTA_RED 8 + INVARIANT_GREEN 5、§6） |
...
| **SPEC-D-5** | **byte-identical 不適用**；驗收分兩類（Arch R2）：**`DELTA_RED`**（每個新增行為 delta）base RED→candidate GREEN；**`INVARIANT_GREEN`**（no-weakening / policy-preservation）base GREEN∧candidate GREEN。**禁**把 invariant 測試逼成 pre-fix RED（邏輯不可能）。§6 = 13 cases（DELTA_RED 8 + INVARIANT_GREEN 5） | **§6** |
 ...
 └─ else
        → URL.searchParams.get(...)                             → string | null
              ...
              [USERINFO_FETCH]  GET cfg.userInfoUrl（Bearer）      ✅ 冪等純讀（apple 不到此）
                    ...
                    │       〔timeout（fetch 階段 **或** body-read 階段，didTimeout flag）／ network（fetch-reject）／ resolved 5xx〕
...
- **⚠ 核心不變式（分類靠「phase flag + 結構」、**零 error 形狀 introspection**；round-2 抗脆弱 + Arch R1 body-timeout）**：timer callback 設 `didTimeout=true` 再 `ctrl.abort()`；catch 包 **fetch + `res.json()`**（故 body-stall 的 timeout 也進 retry，**Arch R1 修正**）。retry 判定 = **`didTimeout || res === undefined`**：
  ...
  - `res === undefined` 純結構（fetch 階段 vs body 階段）：fetch-reject（network，res 未設）→ transient；**body-json-reject 且非 timeout（malformed）→ terminal**（不誤 retry 壞 body）。
  - 4 種 reject 全覆蓋：fetch-timeout〔didTimeout✔〕· fetch-network〔res===undefined✔〕· body-timeout〔didTimeout✔，**Arch R1 新納入 retry**〕· body-malformed〔兩者皆 false → terminal〕。
...
- **四 transient trigger 各有 test**：5xx = resolved `res.status>=500`（T5 rescue / T5b 耗盡）；fetch-timeout（T8）；**body-timeout（T8b、Arch R1）**；network（T9）。**T8/T8b/T9 的 mock 只需 honor abort/reject（任何形狀）**（見 §6 mock 契約）。
...
| guard 誤擋合法 callback（regression） | **精準回退 guard hunk**（L76 guard + L61 分支相關），**保留 fetch 韌性 + exchangeCode 標型**；T3(apple)/既有 suite 應先攔到 |

**完整 `git revert` 不是首選**（會重引已實證的 D1 500 / state 提前核銷 / 4×TS7031）。
...
## 6. Negative / regression test 清單（SPEC-D-5；**13 cases、兩類**）
...
- **`INVARIANT_GREEN`（no-weakening / policy-preservation）**：**base GREEN ∧ candidate GREEN**（證既有行為不被弱化）。＝{ T3, T6, T6b, T7, T10 }（5 條）。
- 合計 **13 cases**（含 T5b/T6b/T8b；不再誤稱「T1–T10」）。
...
| **T4** | DELTA_RED | discord | tokenUrl mock(i) 掛住 | RED：無限等 → vitest 20s testTimeout | `OAUTH_FETCH_TIMEOUT_MS='50'` ⇒ abort → 400 · 耗時 < 1s |
...
> **INVARIANT_GREEN 的 delta 佐證**：T6/T6b/T7/T10 在 base 與 candidate 皆 GREEN、**斷言值不變**（次數/狀態相同）⇒ 證本 PR **未弱化**這些既有 no-retry / verify-first policy（Arch #7）。它們**不是** delta、故不可能 pre-fix RED；把它們列入 DELTA_RED 是原 SPEC-D-5 的邏輯矛盾。
...
> ⚠ scout 的 `zz-b5-probe.test.ts` 是**一次性證據、未進任何 commit**（throwaway worktree 已移除）。13 cases 必須在本 PR **正式落地**成 `tests/integration/` 交付物；**DELTA_RED 8 條須留「base RED」輸出證據、INVARIANT_GREEN 5 條須留「base GREEN ∧ candidate GREEN」證據**（SPEC-D-5 兩類）。
...
- **兩類驗收實證（SPEC-D-5、Arch R2）**：**DELTA_RED 8 條**（T1/T2/T4/T5/T5b/T8/T8b/T9）須留「在 base `274a37b4` 上真的 fail」證據；**INVARIANT_GREEN 5 條**（T3/T6/T6b/T7/T10）須留「base GREEN ∧ candidate GREEN」證據（證未弱化既有 policy）。禁把 invariant 逼成 RED。
