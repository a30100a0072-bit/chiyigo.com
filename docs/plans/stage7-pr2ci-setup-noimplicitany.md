...
- 2026-06-19 **ChatGPT Faithfulness Gate：`CHATGPT_CODE_FAITHFULNESS_APPROVED`** @ source `d7ae6753`（0 blocker / 0 required revision / 1 NB）— 9 核對項全 faithful（scope / edit point / error target / runtime byte-identical / hot-zone lock / env lock / test lock / coverage claim / gate lock）。NB-1：merge 前仍須重跑 merge-front local gates（Codex `test:int` 1328 為強佐證、不取代最終 replay）。**外部 4 gate 全通過。**
- 2026-06-19 **merge-front local gates（@ HEAD `70713e39`，CI-equivalent 全綠，閉合 NB-1）**：`lint` 0 · `typecheck:ratchet` OK（base `176bf542`、current **831/246**、ceiling 1119/175）· `verify:browser-pipeline` OK（25 pages / 214 refs）· `test:cov` exit 0（Statements **90.28%**）· `test:int` **75 files / 1328 tests 全通過**（self-replay 確認 Codex 結果）· `build:functions` compiled · `npm audit --omit=dev --audit-level=high` **0 vulnerabilities**。
- **owner `MERGE_ALLOWED`**：PENDING（外部 4 gate + merge-front 全綠；待 owner 明示才 squash-merge `--delete-branch`）。
