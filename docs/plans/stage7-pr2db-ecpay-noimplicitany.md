...
- [x] `CODEX_CODE_APPROVED`（③ 維度 C、2026-06-29、**0 blocking/major/minor**）；Codex 親從 **live repo** 驗：source `b05f4d8a` 只改 ecpay.ts +32/−13、HEAD `9e9ea544`·`HEAD:ecpay.ts`==`5f3ebb3b`、base `1ab7b625`→committed `5f3ebb3b`、committed source diff == frozen byte-for-byte〔sha `118bf104…`/5910B〕、forced tsc archive replay 700→673·ecpay 27→0·REMOVED 27/ADDED 0、byte-identical **非恆真** base==committed `04b32226…`/11354B、**no escape hatch**〔無 `as any`/`as Env`/`as unknown`/`@ts-ignore|expect-error|nocheck`〕。**payment security clean**〔CheckMacValue/getCreds fail-closed/webhook parse/refund verify/sandbox creds/response shape runtime-unchanged〕。Codex verify run：ratchet 673/272·lint·build:functions〔wrangler compiled〕·browser-pipeline·test:cov 90.28%〔25f/737〕·targeted worker int 3f/91〔payments-ecpay 23/23〕·audit 0〔Codex 未重跑全量 75-file test:int、跑 payments-targeted path；**main 線已實跑全量 75f/1328**〕。明示**非 merge 授權**）→ 待送 ④ ChatGPT Faithfulness
- [x] `CHATGPT_CODE_FAITHFULNESS_APPROVED`（④ 維度 B-code、2026-06-29、**FAITHFUL**：0 blocker / 0 required revision / 0 faithfulness finding、**不 invalidate ③**）；faithfulness matrix 全 Faithful〔Scope single-file / OD-PR2db-1 `Pick<Env>` / 不採 full Env / 不標 const PaymentAdapter / `WebhookParseResult` return / ARCH-L14 Pick 精確 5 key / 不改 env.d.ts·PAY-002·integration·payment-types·payments / no cast·no lexical any / payment_info runtime·test 不改 / byte-identical〕、**anti-curated name-status 反問 clean**〔source 僅 `M ecpay.ts` 附完整 hunk、無「改動未附 hunk」source 檔、排除檔皆不在清單〕、committed==frozen sha `118bf104…`、PSP Tier-0 紅線無變更、ARCH-L12..L18 全符。**外部 4 道全過（①②③④）** → ⏳ owner `MERGE_ALLOWED`（待明示）；merge 前重跑 merge-front 7 gates〔含全量 test:int〕
