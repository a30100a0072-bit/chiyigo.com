...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ✅ `CHATGPT_ARCH_APPROVED_WITH_LOCKS`（① 維度 B、**0 Blocker / 0 Required Revision / 1 NB**、binding locks LOCK-1..11、全對齊本 plan L1..L13 + frozen diff、無 plan 改動 — 見 §Gate 進程紀錄）→ ⬜ `CODEX_PLAN_APPROVED`（② 維度 C）→ ⬜ owner `CODING_ALLOWED`
  - ⬜ Code 階段（`CODING_ALLOWED` → source commit、full replay @ committed、NB-2 雙證）→ ⬜ `CODE_SELF_REVIEW_CLEAN`（維度 A workflow）→ ⬜ `CODEX_CODE_APPROVED`（③ 維度 C）→ ⬜ `CHATGPT_CODE_FAITHFULNESS_APPROVED`（④ 維度 B-code）
...
- ⬜（後續 dated 收錄：plan doc commit〔gate-log〕 → Codex Plan packet〔HEAD-independent anchor〕 → ② Codex Plan → owner CODING_ALLOWED → Code 階段 …）
