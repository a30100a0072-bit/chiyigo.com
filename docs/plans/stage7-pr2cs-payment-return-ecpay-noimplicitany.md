...
- ⬜（後續 dated 收錄：owner `CODING_ALLOWED` → Code 階段 source commit → full replay @ committed〔NB-2 雙證〕 → code self-review workflow → ③ Codex Code → ④ ChatGPT Faithfulness → merge-front 7 gates → owner `MERGE_ALLOWED` → squash → SHIPPED）
