...
- （後續 dated 收錄：② Codex Plan → `CODING_ALLOWED` → Code（落地 + full replay @ source）→ ③ Codex Code → ④ ChatGPT Faithfulness → merge-front 7 gates → `MERGE_ALLOWED` → squash → main CI / deploy → SHIPPED memory）
