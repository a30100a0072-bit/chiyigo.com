...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ⬜ `CHATGPT_ARCH_APPROVED`（① 維度 B；owner 驅動，未送）
...
- （後續 dated 收錄：plan commit → 中文報告 → owner 驅動 ① ChatGPT Arch → ② Codex Plan → `CODING_ALLOWED` → Code → ③ Codex Code → ④ ChatGPT Faithfulness → merge-front → `MERGE_ALLOWED` → squash → main CI / deploy）
