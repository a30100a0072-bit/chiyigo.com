...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ⬜ Code 階段（落地 + full replay @ source）→ ⬜ `CODE_SELF_REVIEW_CLEAN`（multi-agent workflow）→ ⬜ `CODEX_CODE_APPROVED`（③ 維度 C）
...
- （後續 dated 收錄：owner `CODING_ALLOWED` → Code（落地 + full replay @ source、不沿用 spike）→ `CODE_SELF_REVIEW_CLEAN`（multi-agent）→ ③ Codex Code → ④ ChatGPT Faithfulness → merge-front 7 gates → `MERGE_ALLOWED` → squash → main CI / deploy → SHIPPED memory）
