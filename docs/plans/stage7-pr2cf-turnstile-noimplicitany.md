...
當前 state = **`CODEX_PLAN_APPROVED`（@ plan `f5590ae0`）→ 待 owner `CODING_ALLOWED`**。impl **L1** / review care **L2**。**Plan-stage 兩道外部 gate（ChatGPT Arch + Codex Plan）全過、8 Required Locks 全 enforce。未到 `CODING_ALLOWED`，不得改 source、不開 merge path。**
...
- **待**：owner 明示 **`CODING_ALLOWED`** → code 階段（frozen replay +5/−1）。本 PR **尚未進入 code 階段、未開 merge path**。
