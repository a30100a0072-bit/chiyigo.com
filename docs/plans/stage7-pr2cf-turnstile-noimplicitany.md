...
當前 state = **`MERGE_ALLOWED`（@ source `5d4605d6`）→ 待 owner 明示 squash-merge**。impl **L1** / review care **L2**。**基本 4 道外部審查全過（ChatGPT Arch + Codex Plan + Codex Code + ChatGPT faithfulness）+ 機械 gates 全綠 + byte-identical 實證。待 owner 明示 `squash-merge --delete-branch`；未到位前不 push / 不 merge / 不動 main / 不 `baseline:update`。**
...
- 2026-06-18 **plan-doc commit**（feature branch `stage7-pr2cf-turnstile-noimplicitany` @ `cc1828db`，docs-only +175、`git diff --check` clean、`CLEANUP_PLAN.md` 未入）。
- 2026-06-18 **ChatGPT Architecture Gate（維度 B，owner-relayed）：`CHATGPT_ARCH_APPROVED_WITH_LOCKS`** — 0 Blocker / 0 Required Revision / **8 Required Locks**（L-1..L-8）。全數已在既有 plan §Coding 硬性邊界 / §敏感面 / §驗證計劃 / §測試影響面 enforce → **0 架構/scope/typing change**，僅納為顯式驗收條目；OD-1/OD-2 經 ChatGPT Arch 確認「符合前裁示」。L-1..L-8 mapping：

| Lock | 要求 | 既有 enforce 位置 |
|---|---|---|
| L-1 scope | 只 replay frozen diff（3 typed params）| §Coding 硬性邊界「允許」+ §Spike 最終 diff |
| L-2 runtime | 不改 fetch / URLSearchParams / data parsing / return / catch reason | §Coding 硬性邊界「禁止」+ §Runtime 行為不變保證 |
| L-3 fail-close | siteverify throw/unreachable → `{ ok:false, reason:'siteverify_unreachable' }` | §敏感面 fail-close 語意 #4 |
| L-4 skip-path | `!TURNSTILE_SECRET_KEY` → `{ ok:true, skipped:true }` | §敏感面 fail-close 語意 #1 |
| L-5 coverage wording | 不宣稱 direct test；只 3 caller skip-path 間接 | §測試影響面「覆蓋宣稱」 |
| L-6 baseline | 不 `--update`；天花板 1119/175 | §預期 ratchet + §驗證計劃 |
| L-7 同名函式 | 不碰 `api/ai/assist.ts` local helper | §Coding 硬性邊界 + §caller proof「明確排除」 |
| L-8 scope-exclusion | 不碰 data:any / return type / JSDoc / caller / tests / config / totp / CLEANUP_PLAN | §Coding 硬性邊界「禁止」+ §Open Decisions |

- 2026-06-18 **Codex Plan Gate（維度 C，owner-relayed）：`CODEX_PLAN_APPROVED`**（@ plan `f5590ae0`）— 0 blocker / 0 required revision。Codex 獨立對帳：merge-base main `3bd26315`、`main..HEAD` docs-only（僅 plan 檔、無 `functions/` diff）、`turnstile.ts` blob `79afadc7`（HEAD==main 未動）、frozen 簽名 diff in-memory replay → `049b0dc5`（無 source 寫入）、`typecheck:ratchet:report` 850/96/238、filtered tsc 確認恰 3×TS7006（31,39/48/54）、`git diff --check main..HEAD` clean、coverage wording 誠實（無 direct turnstile test、tests/config 無 `TURNSTILE_SECRET_KEY`/`cf-turnstile-response`）。Codex 明示「只批 plan、非 Code Gate；next valid source step = frozen +5/−1 replay only + 機械 gates + fresh code-stage byte-identical receipt」。
- 2026-06-18 **owner `CODING_ALLOWED`** → Code 階段。frozen replay：working-tree diff 對 plan §Spike frozen **逐行 byte-identical**（1 檔 +5/−1、blob `79afadc7→049b0dc5`、僅 turnstile.ts）。**source commit `5d4605d6`**。
- 2026-06-18 **機械 gates 全綠（full replay @ source `5d4605d6`，clean `.tscache`、不沿用 spike）**：`RATCHET_BASE_REF=3bd26315 typecheck:ratchet` **OK**（current **847/239**、baseline **1119/175** 不動、committed-diff banned-pattern scan clean）／`tsc -p tsconfig.functions.json` turnstile **0** residual（total 847）／`lint` exit 0（eslint functions tests + compat-date + workflows）／`build:functions` Compiled Worker successfully／`test:cov` exit 0（**737 passed**、cov 90.28%）／`test:int` **75 files / 1328 passed**（含 register/login/forgot-password skip-path 間接 regression、無 spillover、502s）。`git diff --check` clean。
- 2026-06-18 **emit byte-identical 實證（fresh @ code stage，非僅 plan-stage）**：base(`3bd26315`) vs HEAD 的 `turnstile.ts` esbuild stdin type-strip JS **逐位元組相同**（皆 **955 bytes、sha `1924bcbc…`**、非空）→ runtime bytecode 證實不變（含未被 test 覆蓋的 fail-close 路徑）。
- 2026-06-18 **`CODE_SELF_REVIEW_CLEAN`（單 agent 對抗式，@ source `5d4605d6`，impl L1，一輪 0 新發現）**：見 §流程定位。
- 2026-06-18 **Codex Code Gate（維度 C，owner-relayed）：`CODEX_CODE_APPROVED`**（@ source `5d4605d6`）— 0 blocker / 0 required revision。Codex 獨立復現：frozen replay faithful（blob `79afadc7→049b0dc5`、+5/−1、單簽名 3 param annotation）、`main..HEAD` runtime/test/config surface 僅 turnstile.ts（docs plan 為唯一另一改動檔）、`git diff --check` clean、`RATCHET_BASE_REF=3bd26315 typecheck:ratchet` OK（847/239、baseline 1119/175）、`tsc -p tsconfig.functions.json` turnstile 0、`eslint`/`lint` OK、`build:functions` compiled、`test:cov` 737/90.28%、`test:int` 75 files/1328 passed（489.93s）、esbuild base==head 955 bytes/sha `1924bcbc…` identical。coverage caveat（無 direct test、依 byte-identical）對 type-only diff 判定 sufficient。**只批 code、非 release。**
- 2026-06-18 **ChatGPT Code Faithfulness Gate（維度 B，owner-relayed）：`CHATGPT_CODE_FAITHFULNESS_APPROVED`**（@ source `5d4605d6`）— **FAITHFUL**、0 Blocker / 0 Required Revision / 0 Non-blocking。逐項：source scope 僅 turnstile.ts ✅／docs scope plan-doc ✅／frozen diff 只補 3 param 型別 ✅／OD-1 未加 return type ✅／OD-2 未清 JSDoc ✅／runtime emit byte-identical 955 bytes sha `1924bcbc…` ✅／fail-close 語意（token_missing / verify result / siteverify_unreachable）未漂移 ✅／caller(register/login/forgot-password) 未動 ✅／`api/ai/assist.ts` 同名 helper 未動 ✅／tests / baseline / config 未碰 ✅／未宣稱 direct Turnstile coverage ✅／無 scope creep（data:any / return object / fetch / URLSearchParams / catch 未碰）✅。
- **`MERGE_ALLOWED`**：基本 4 道外部審查全過（ChatGPT Arch + Codex Plan + Codex Code + ChatGPT faithfulness）+ 機械 gates 全綠 + emit byte-identical 實證。**待 owner 明示 `squash-merge --delete-branch` go**；未到位前不 push / 不開 PR / 不 merge / 不動 main / 不 `baseline:update`。
...
- **Claude CODE 自審紀錄（`CODE_SELF_REVIEW_CLEAN`，單 agent 對抗式，@ source `5d4605d6`，impl L1，一輪 0 新發現）**：對抗——(1) faithful replay：post-commit blob `049b0dc5` == frozen post-blob、+5/−1、僅 turnstile.ts（`git diff --name-only main..HEAD -- functions/**` = 單檔）✅；(2) scope：totp / assist / caller(register/login/forgot-password) / tests / config / baseline / `CLEANUP_PLAN.md` 0 命中（grep 證）✅；(3) runtime-invariance：**emit byte-identical 實證**（base vs HEAD type-strip JS 皆 955 bytes/sha `1924bcbc…`）+ build:functions compiled + test:int 1328/1328 廣度 regression（無他檔 spillover）+ test:cov 737 ✅；(4) 敏感面 byte-identical：fail-close 四語意（skip/token-missing/verify/catch）+ SITEVERIFY_URL + fetch + URLSearchParams + data 解析全在 diff 行外 ✅；(5) 無禁用 pattern：無 `:any`/`as any`/suppression/新 import/新 runtime 分支、eslint 0、committed-diff banned-pattern scan clean（自掃 0 命中）✅；(6) typing 忠 caller：`Request`/`Record<string,unknown>`/`Pick<Env,'TURNSTILE_SECRET_KEY'>`——3 caller 皆傳 implicit any、test:int 1328 綠證無 breakage ✅；(7) runtime 誠實：byte-identical 非僅推斷、esbuild 實測非空（避 `--loader` 空輸出陷阱）✅；(8) ratchet honesty：報 current 847/239、baseline file 未 `--update`（1119/175 凍結）✅；(9) coverage 誠實：turnstile 不在 cov 報告（無 direct test 確認）、僅 3 caller skip-path 間接、不宣稱 direct ✅。
