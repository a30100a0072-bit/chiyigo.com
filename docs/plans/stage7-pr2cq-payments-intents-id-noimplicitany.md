...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ⬜ `CHATGPT_ARCH_APPROVED*`（① 維度 B）→ ⬜ `CODEX_PLAN_APPROVED`（② 維度 C、HEAD-independent anchor packet）→ ⬜ owner `CODING_ALLOWED`
...
- （後續 dated 收錄：plan doc commit → 中文報告 → owner 驅動外部 ①②③④ → CODING_ALLOWED → Code → merge-front → MERGE_ALLOWED → SHIPPED）
