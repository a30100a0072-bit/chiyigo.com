...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ⬜ `CHATGPT_ARCH_APPROVED`（維度 B，owner 驅動）
...
- （後續 dated 收錄：plan commit → 中文報告 → ChatGPT Arch → Codex Plan → CODING_ALLOWED → Code → Codex Code → ChatGPT Faithfulness → merge-front gates → MERGE_ALLOWED → squash → main CI / deploy）
