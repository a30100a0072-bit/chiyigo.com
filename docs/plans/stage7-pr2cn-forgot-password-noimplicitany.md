...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ✅ `CHATGPT_ARCH_APPROVED_WITH_LOCKS`（① 維度 B，**0 required revision / 2 NB**、8 維全 PASS、binding locks CL-1..CL-10 — 見 §Gate 進程紀錄）→ ⬜ `CODEX_PLAN_APPROVED`（② 維度 C，owner 驅動）→ ⬜ owner `CODING_ALLOWED`
  - ⬜ Code 階段（source commit + full replay @ source，不沿用 spike、NB-2 雙證）→ ⬜ `CODE_SELF_REVIEW_CLEAN`（multi-agent workflow）→ ⬜ `CODEX_CODE_APPROVED`（③）→ ⬜ `CHATGPT_CODE_FAITHFULNESS_APPROVED`（④）
...
- （後續 dated 收錄：② Codex Plan → `CODING_ALLOWED` → Code 階段 full replay → ③ Codex Code → ④ ChatGPT Faithfulness → merge-front 7 gates → `MERGE_ALLOWED` → squash-merge → SHIPPED memory）
