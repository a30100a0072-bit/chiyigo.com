...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ⬜ `CHATGPT_ARCH_APPROVED`（① 維度 B，owner 驅動）→ ⬜ `CODEX_PLAN_APPROVED`（② 維度 C，owner 驅動）→ ⬜ owner `CODING_ALLOWED`
...
- （後續 dated 收錄：plan doc commit → 中文報告 6 欄 → owner 驅動 ① ChatGPT Arch packet → ② Codex Plan → `CODING_ALLOWED` → Code 階段 full replay → ③ Codex Code → ④ ChatGPT Faithfulness → merge-front 7 gates → `MERGE_ALLOWED` → squash-merge → SHIPPED memory）
