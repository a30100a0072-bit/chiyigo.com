...
- **gate 狀態（前瞻 checklist；外部 gate 尚未送、不得自我宣告通過）**：
  ...
  - ✅ Code 階段（source commit `1ef28c7f`、full replay @ source 全綠、NB-2 雙證）→ ✅ `CODE_SELF_REVIEW_CLEAN`（multi-agent workflow `wf_bffb0a00-883`，3 agents 三維、0 confirmedReal defect）→ ✅ `CODEX_CODE_APPROVED`（③，**0 blocking / 0 required**、機械重驗全綠 + int 9/9、commit chain 乾淨 — 見 §Gate 進程紀錄）→ ⬜ `CHATGPT_CODE_FAITHFULNESS_APPROVED`（④，owner 驅動）
...
- （後續 dated 收錄：④ ChatGPT Faithfulness → merge-front 7 gates → `MERGE_ALLOWED` → squash-merge → SHIPPED memory）
