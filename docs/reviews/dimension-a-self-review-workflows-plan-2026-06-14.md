...
- secret denylist（禁止讀取；此 literal 引用為 §8.1 required，**非**違規）：.env* / .dev.vars / .canary-* / .claude/settings.local.json。
```
...
secret denylist ＝ `.env*` / `.dev.vars` / `.canary-*` / `.claude/settings.local.json`。lint 必須區分兩種出現：
