...
> 狀態：`PLAN_DRAFT`（**Rev 2**）→ 重送 ChatGPT Architecture Gate。分級：**L2**。
...
```
**`package.json` 改（CI 接入，不改 ci.yml）**：
```
"lint:workflows": "node scripts/lint-workflows.mjs",
"lint": "eslint functions tests && npm run lint:compat-date && npm run lint:workflows"
```
> CI 既有 `npm run lint` step（ci.yml L30）→ 自動跑 `lint:workflows`，達成 OD-A 的 CI / pre-merge enforce，不需動 `.github/workflows/*`。
...
| **secret denylist（B2/B3）** | 腳本/prompt 不得引用 `.env*`/`.dev.vars`/`.canary-*`/`settings.local.json` |
...
- secret denylist：腳本/prompt/evidence_path 三處皆禁 `.env*`/`.dev.vars`/`.canary-*`/`settings.local.json`。
...
`PLAN_SELF_REVIEW_CLEAN`（Rev 2）。架構盲點（B2/B3）已由 ChatGPT Arch Gate 補上 → 重送 Arch Gate 確認。
