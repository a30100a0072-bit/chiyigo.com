...
**請 codex r4 review**：（a）r3 全 2 件 finding 是否正確套到位；（b）§4.3 forensic 改用 `scripts/audit-archive-forensic-classify.mjs` 走 inline-port + parity test（[`tests/audit-archive-forensic-classify-parity.test.ts`] 27/27 PASS，鎖死 canonical drift）；（c）§1.4 T1 已 strip cost-pressure 為 waiver 理由，改要求 separate risk-acceptance artifact 或 defer 到 2026-06-10；（d）`.mjs` helper 走 inline-port 是否能被 codex 接受（vs codex r3 wording 「via deriveKeysFromChunk / deriveAggregateKeysFromChunk」嚴格用詞 — 加 deps 受 [[CLAUDE.md]] §套件管理 限制；parity test 替代 import 等同性的證明）。
...
## Changelog: r3 → r4（2026-05-28）

Codex r3 verdict = **Reject for Phase 3**。2 件 finding 全收：

| # | Finding 等級 | r3 抓到 | r4 修法 |
|---|---|---|---|
| 12 | **Blocker** | §4.3 Step 2 D1 cross-reference SQL 用 `chunk_key` / `manifest_key` column 不存在於 schema（`tests/integration/_setup.sql:206`）；keys 是 `audit-archive.ts:379` `deriveKeysFromChunk` runtime derive 出來的 | r4 新增 `scripts/audit-archive-forensic-classify.mjs` — inline-port `deriveKeysFromChunk` + `deriveAggregateKeysFromChunk` + `archivePrefixes` + `aggregatePrefixes` + `manifestSuffix` 全 4 helpers，覆蓋 raw + aggregate × scheme=1/2 × dry-run × compression all combos；+ `tests/audit-archive-forensic-classify-parity.test.ts` 27 cases 鎖死 canonical vs inline-port drift（任一公式 drift → 紅燈）。§4.3 Step 2 改寫成「dump chunks JSON → 跑 mjs CLI → 看 matches」3 步流程 |
| 13 | **High** | T1 dry-run < 1 month 條件，r3 允許 walk-through inline waiver + cost-pressure 為 fall-back 理由 — 對 SoT trigger（line 781「全部滿足」）放鬆過頭 | r4 §1.4 T1：(1) cost-pressure 從 waiver 理由 strip 掉；(2) waiver 必須走 **separate explicit risk-acceptance artifact**（建議路徑：新 `docs/decisions/pr-0-2c-dry-run-waiver-2026-05-28.md` doc + 補 git 紀錄；OR 同 PR 直接 amend `docs/AUDIT_RETENTION_PLAN.md:781` SoT 條文），不可只在 walk-through inline；(3) **預設 path = defer 到 2026-06-10**（natural compliance）；要 waiver 必明示書面決策 |

**請 codex r4 驗**：r3 兩件 finding 是否正確套到位；mjs inline-port + parity test 是否能取代 codex r3 wording 中「via deriveKeysFromChunk / deriveAggregateKeysFromChunk」嚴格用詞（CLAUDE.md §套件管理 禁加 tsx/ts-node 為 helper script）；T1 改 separate-artifact-or-defer 是否真切堵死 SoT 漏洞。
...
1. **書面形式**：寫進 **separate explicit risk-acceptance artifact**，不可只在 Phase 2 walk-through inline 口頭同意。建議路徑：
   - (a) 新增 `docs/decisions/pr-0-2c-dry-run-waiver-2026-05-28.md` decision doc，含：時間戳 / 簽核人 / 風險描述 / 替代驗證證據 / accepted residual risk / 撤回觸發條件
   - (b) 同一 PR 直接 amend `docs/AUDIT_RETENTION_PLAN.md:781` SoT 條文（如把「≥1 month」改成「≥2 weeks + binding canary PASS」），git history 永久紀錄
   - (c) 寫進 [[project_audit_phase2]] 加 waiver section
   - 任一以上滿足即可，但**必須在 Phase 3 cmd 開跑前** commit 到 git
2. **waiver 理由禁列項**（codex r3 明示）：
   - ❌ Cost pressure / $0 成本壓力 / D1 query budget consumption — 不可作為 primary safety-waiver reason
3. **waiver 理由可列項**（建議 user 自評）：
...
- **Path B (waiver)**：commit 上述 1 artifact → Phase 2 walk-through user 對著 artifact 對齊每條 trigger → 5/5 trigger ✅（T1 with explicit waiver reference）→ Phase 3 動工。**多 commit 1 artifact，但時程不延**。

Phase 1 不替 user 選擇；codex r4 review 也不替 user 選擇。Phase 2 walk-through 是 user 自選 path 的時點。
...
**Step 2 — Classify provenance via D1 cross-reference**（codex r3 finding 12 套用）：

**Why not SQL where chunk_key = ?**：`audit_archive_chunks` schema **沒有** `chunk_key` / `manifest_key` column（驗 `tests/integration/_setup.sql:206-234`）。Keys 是 runtime derive 出來的（`audit-archive.ts:379` `deriveKeysFromChunk` / `audit-aggregate-archive.ts:248` `deriveAggregateKeysFromChunk`）— 必須用對應的 derive 公式反向比對。

走 `scripts/audit-archive-forensic-classify.mjs` helper（inline-port canonical derive + 27-case parity test 鎖死 drift）：

```powershell
# (1) 全 chunks 表 dump 成 JSON
& $wrangler d1 execute chiyigo_db --remote `
  --command "SELECT env, table_name, cold_class, archive_date, min_id, max_id, chunk_sha256, dry_run, compression, key_scheme FROM audit_archive_chunks" `
  --json > "docs/fixtures/forensic-chunks-dump-2026-05-28.json"

# (2) 對每個被檢出的 key 跑 forensic classify
$KEY = '<full-key-path-from-Method1-Contents-array>'
node scripts/audit-archive-forensic-classify.mjs `
  --found-key=$KEY `
  --chunks-json=docs/fixtures/forensic-chunks-dump-2026-05-28.json `
  > "docs/fixtures/forensic-classify-$($KEY -replace '/', '__').json"
```
...
```json
{
  ...
  "matches": [
    {
      "row": { "env":"prod", "table_name":"...", ..., "key_scheme": 2 },
      "key_kind": "raw_data" | "raw_manifest_planned" | "raw_manifest_uploaded" | "raw_manifest_verified" | "raw_manifest_marked_archived" | "raw_manifest_legacy" |
                  "aggregate_data" | "aggregate_manifest_planned" | "aggregate_manifest_uploaded" | "aggregate_manifest_verified" | "aggregate_manifest_marked_archived" | "aggregate_manifest_legacy",
...
**Decision matrix per matches output**：
- `matches.length >= 1` → D1 reference 找到 → 直接套 Step 3 Row 1（**NEVER delete + incident**）
- `matches.length === 0` AND key pattern 含 test/sacrificial marker → Step 3 Row 2（**user explicit approval required**）
- `matches.length === 0` AND key pattern 不明 → Step 3 Row 3（**保守 incident path**）

**Helper script drift 防護**：[`tests/audit-archive-forensic-classify-parity.test.ts`] 27 cases 對齊 canonical TS function；CI 任一公式 drift → 紅燈 → 修 mjs 或修 canonical。本 helper 修改 PR 必同步 parity test，否則 Phase 3 不應動工。

**為何不直接 import canonical TS function**：CLAUDE.md §套件管理 禁加 tsx / ts-node 為 helper 用途；本機 Node 22 `--experimental-strip-types` 對 extension-less import 不穩；inline-port + parity test 是 0-dep 最小 trade-off。
...
- 若 codex 後續 round 在 §7 任何 open question 給出推翻本版的答案，更新本 plan + AUDIT_RETENTION_PLAN.md 對應段
...
- **(r4 衍生，codex r3 finding 12)**：`scripts/audit-archive-forensic-classify.mjs` + `tests/audit-archive-forensic-classify-parity.test.ts` 兩檔屬「forensic 工具」屬性 — Phase 3 完成且 14-day no-incident watch 過後，評估：(a) 保留作 future incident response 工具、(b) 抽進 [[reference_audit_archive_forensic_helper]] reference memory、(c) 若 audit-archive code 變更導致 parity test 紅燈，立刻同步 inline-port
- **(r4 衍生，codex r3 finding 13)**：若 user 選 T1 waiver path，artifact 路徑（`docs/decisions/pr-0-2c-dry-run-waiver-2026-05-28.md` 或 AUDIT_RETENTION_PLAN.md amend）必獨立 commit；archive 完成後追記到 [[project_audit_phase2]] 結案 section 標明「T1 SoT trigger waived via <artifact>」
