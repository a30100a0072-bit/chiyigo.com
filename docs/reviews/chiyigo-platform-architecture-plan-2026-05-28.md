...
- **建立日期**：2026-05-28
- **狀態**：DRAFT — codex r1–r4 findings 已 patch（deductCredits 加欄位 NOT NULL + quota-row missing guard）。**PR1 Tenant Foundation = codex Approved to start** ✅；full design gate 待 r5 複驗。§0.2 五項已 user 拍板（2026-05-28）
...
> **r5 焦點**：r4 blocker（quota 0-row success / NULL bypass）已修 —— wallet/quota 欄位 `NOT NULL DEFAULT + CHECK`，`credit_ledger` 加 `quota_used_after/quota_limit_after NOT NULL`（subquery 讀 quota row，漏建 → NULL → rollback）。**PR1 已 Approved**。請複驗是否達 full gate。

**r5 複驗點**：§5 quota/wallet 欄位 NOT NULL（堵 SQLite CHECK-on-NULL 繞過）+ `*_after` NOT NULL subquery 堵「row missing」0-row success → 不足/超額/漏建任一路徑必 statement error → 整批 rollback。
