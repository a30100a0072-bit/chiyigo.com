...
- **建立日期**：2026-05-28
- **狀態**：DRAFT — codex r1(7)+r2(5)+r3(deductCredits CAS-abort + grant CHECK) 已 patch。**PR1 Tenant Foundation = codex Approved to start** ✅；full design gate 待 r4 複驗最後修補。§0.2 五項已 user 拍板（2026-05-28）
...
- `credit_wallets`（新，**單 tenant 一個錢包，決策③**）：`tenant_id(PK)`、`balance(int)`、`version`、`updated_at`
  - **`CHECK(balance >= 0)`**（codex r3：扣到負 → statement error → batch rollback，見下方 deductCredits）
- `product_usage_quota`（新，決策③）：`tenant_id`、`product_id`、`quota_limit(int)`、`quota_used(int)`、`period`、`version`、`updated_at`
  - unique：`(tenant_id, product_id, period)`；**`CHECK(quota_used <= quota_limit)`**（codex r3：超額 → statement error → rollback；相對更新 + CHECK 取代 conditional `WHERE`，因 0-row 不 abort）
...
- schema 約束：`credit_wallets CHECK(balance >= 0)`、`product_usage_quota CHECK(quota_used <= quota_limit)`、`credit_ledger.balance_after NOT NULL`。
- 單一 batch（任一語句 error → 整批 rollback）：
  ...
  3. `INSERT INTO credit_ledger (…, balance_after=(SELECT balance FROM credit_wallets WHERE tenant_id=?), idempotency_key, request_hash, …)` → 重複 key → **unique 違反 → error → rollback**（並發雙扣只成功一次）。
- **相對更新 + CHECK 取代 version-CAS**：SQLite 寫入序列化，`balance = balance - ?` 為相對更新，靠 CHECK 擋負值即正確且並發安全，無需 version 重試迴圈（`version` 欄位保留給 admin adjust 等其他路徑）。
- **idempotency 前置查 + 唯一鍵雙保險**：先查 ledger 同 key → 命中回上次結果（`request_hash` 不符 → 409）；並發漏網由第 3 步唯一鍵擋（敗方整批 rollback → re-read 回上次結果）。
- **wallet/quota row 必在 provision 時建好**（subquery 抓不到 → `balance_after` NULL → NOT NULL error → rollback，防漏建）。
- **對帳 backstop**：定期 job 驗 `wallet.balance == Σtopup − Σdeduct(ledger)` 且 `quota_used == Σdeduct(ledger, product)`；drift → 告警 + 凍結扣點待人工。
- **必測**：並發雙扣同 key（只成功一次）、餘額/配額剛好邊界、**超額/不足必 rollback 無半完成（驗 CHECK→error→rollback，非 changes=0）**、模擬 crash。
...
> **r4 焦點**：r3 唯一 full-gate blocker（deductCredits D1 batch CAS-abort）已改為 **CHECK-constraint 觸發 statement error → rollback**（§5）；grant CHECK 已補。**PR1 已 Approved to start**。請複驗此最後修補是否達 full gate。

**r4 複驗點**：§5 deductCredits `CHECK(balance>=0)` / `CHECK(quota_used<=quota_limit)` / `balance_after NOT NULL` → 不足/超額/漏 row 必觸發 statement error 而 rollback（**非靠 changes=0**）；`grant_plan_operations` CHECK 強制 trigger 必填欄位。
