...
| 狀態 | 觸發條件 | 升態驗證 |
|---|---|---|
| `planned` | D1 SELECT 完成 + sha256 算完 → 寫 manifest 上 R2 | manifest PUT ok |
| `uploaded` | jsonl.zst PUT 完 | R2 PUT ok |
| `verified` | R2 GET 回讀 + sha256 + row_count 比對 ok | sha256 一致 + 行數一致 |
| `marked_archived` | D1 `UPDATE archived_at=NOW() WHERE id BETWEEN ? AND ? AND archived_at IS NULL` | **見下方雙路徑驗證** |
| `purged` | grace period 過後執行 `DELETE WHERE archived_at < NOW() - grace` | DELETE affected_rows + 已 purged row count 合計 = manifest.row_count |
...
9. 月底所有 chunk = purged → 寫月份 manifest + audit.archive.month_completed
```
...
## Codex review 重點（v4 焦點）

1. **R2 versioning fallback 是否守得住** — 沒有 retention lock 時，最小權限 token + runtime 禁 DELETE + break-glass 三層是否足夠抵 prod 操作意外？
2. **5-狀態機對中斷的覆蓋** — `planned / uploaded / verified / marked_archived / purged` 五態 + `cold_copied / failed / blacklisted` 終態是否足夠？特別看：worker crash-after-update-before-state 場景（v4 雙路徑驗證 marked_archived 是否守得住）
3. **purged 升態的 D1 殘留檢查** — `deleted == row_count AND still_archived == 0` 兩條件能否覆蓋 partial DELETE / 部分 row 不在範圍的場景？
4. **R2 寫成功但 D1 audit event 沒寫進**（例：R2 PUT ok 但寫 `audit.archive.marked_archived` 失敗）— manifest state 與 `audit_archive_chunks.state` 是否真能 cover、不依賴 audit event？
5. **跨 chunk hash chain 驗證對 admin_audit_log（Phase 2 不 purge 後）** — admin_audit_log 仍 hot 全留時，cold copy chunk 跨月 prev_hash_of_first_row 是否真能離線串得起來？
