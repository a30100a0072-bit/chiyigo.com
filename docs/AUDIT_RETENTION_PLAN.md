...
> Status: v8.1 draft (codex round-8 finding 修正) · 2026-05-10
...
```json
{
  ...
  "chunks": [
    {
      "chunk_id": "audit-log/prod/audit_log/security_critical/2026/04/01/1234567-1234892-aaaa.jsonl.zst",
      "min_id": 1234567, "max_id": 1234892, "row_count": 326,
      "sha256_zst": "..."
    },
    {
      "chunk_id": "audit-log/prod/audit_log/security_critical/2026/04/02/1234893-1235410-bbbb.jsonl.zst",
...
-- ✅ 對：找連續 terminal prefix 的 max
WITH ordered AS (
  SELECT min_id, max_id, state,
         LAG(max_id) OVER (ORDER BY min_id) AS prev_max
    FROM audit_archive_chunks
   WHERE table_name=? AND cold_class=?
   ORDER BY min_id
)
-- cursor = 最後一個「state in terminal AND (min_id == prev_max+1 OR prev_max IS NULL)」的 max_id
-- 從第一個 non-terminal 或 gap 之前停下
```
