...
- ⚠ **FACTOR-ADD 前端 elevation 接線（現行主線；插隊在回 Stage 7 strict 之前）**：
  ...
  - **狀態**：branch `feat/factor-add-elevation-ux`；plan `docs/audit/factor-add-frontend-wiring-plan.md`：dimension-A self-review ✅（§12）→ Arch Gate r1 REVISE〔C1 transport／C2 wiring〕→ 修（§13；**C1 用既有 `init?is_binding` 回 JSON `{redirect_url}` contract `init.ts:258-261` 解＝frontend-only 不改後端、不移出 scope；C2 改 per-caller wiring test、不新增 jsdom**）→ **ChatGPT Arch Gate r2 ＝ APPROVED**（§14 鎖定條件：後端／migration／`requireFactorAddGrant`／TTL／grant-token-not-in-URL 全鎖）→ **送 Codex Plan Gate**。OD-1..5 最終裁 **A**（OD-3 A-required）。**尚未進 Code 階段**（Codex Plan Gate 過才 coding）。
