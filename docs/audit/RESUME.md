...
- ⚠ **FACTOR-ADD 前端 elevation 接線（現行主線；插隊在回 Stage 7 strict 之前）**：
  ...
  - **狀態**：branch `feat/factor-add-elevation-ux`；plan `docs/audit/factor-add-frontend-wiring-plan.md`：dimension-A self-review ✅（§12，14 accepted 裁決套入）→ **ChatGPT Arch Gate r1 ＝ REVISE**〔C1 `bind_identity` transport／C2 測試 wiring〕→ **已修（§13）**：**C1 用既有 `init?is_binding` 回 JSON `{redirect_url}` contract（`init.ts:258-261`，header 掛 `apiFetch`、導頁獨立）解＝frontend-only 不改後端、不移出 scope**；**C2 改 per-caller wiring test**（node:vm + 最小 DOM + apiFetch spy，沿用 `api-session-revoked.test.ts`，不新增 jsdom）→ **回送 Arch Gate r2 確認 → Codex Plan Gate**。OD-1..5 Arch r1 全裁 **A**（OD-3 A-required）。**尚未進 Code 階段**。
