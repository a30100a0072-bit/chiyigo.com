...
- ⚠ **FACTOR-ADD 前端 elevation 接線（現行主線；插隊在回 Stage 7 strict 之前）**：
  ...
  - **狀態**：branch `feat/factor-add-elevation-ux`；plan `docs/audit/factor-add-frontend-wiring-plan.md`：dimension-A self-review ✅（§12）→ Arch Gate r1 REVISE〔C1 transport／C2 wiring〕→ 修（§13；**C1 用既有 `init?is_binding` 回 JSON `{redirect_url}` contract `init.ts:258-261` 解＝frontend-only 不改後端、不移出 scope；C2 改 per-caller wiring test、不新增 jsdom**）→ **ChatGPT Arch Gate r2 ＝ APPROVED**（§14 鎖定條件：後端／migration／`requireFactorAddGrant`／TTL／grant-token-not-in-URL 全鎖）→ **Codex Plan Gate ＝ CODING_ALLOWED**（§15，無 blocker；非 blocker R5＝Stage 1 same-origin only，`cors.ts` 未含 `X-Factor-Add-Grant`＝另案）。OD-1..5 最終裁 **A**（OD-3 A-required）。**→ Code ✅ 實作完成**（feat `661d76b9` + cache-bust `9a44c02b`）：`obtainFactorAddGrant` helper + `openElevationModal`（dedicated，OD-1）+ 三 caller 帶 `X-Factor-Add-Grant` header + `checkBindResult` ERR_KEY（elevation_required/consumed）+ i18n（dashboard.json 9 key×3 lang）+ api.ts API_ERROR_I18N（8 code×4 lang）+ `tests/dashboard-factor-add-wiring.test.ts`（5 case：3 caller wiring＋OAuth-only＋grant-leak，node:vm 不新增 jsdom）。**gates 全綠**：ratchet 898（0 新增）／lint／unit 692＋cov 90.28%／test:int 1328／build:functions／自審到零。**dimension-A code-self-review workflow ✅**（`wf_f9cb9f8c`，36 findings/31 多為確認；3 actionable 已修：`submitting` in-flight guard／ko i18n／fail-closed test → fix `749ced39`+cache-bust `b0379dad`；plan §16）。**→ 待 Codex Code Gate + ChatGPT plan-faithfulness → squash-merge**（禁直推 main）。
