...
**更新：2026-06-15**（**FACTOR-ADD Stage 1（TOTP/password）✅ MERGED #84 `285b8987`** ＋ **Stage 2（OAuth-only OAuth-reauth elevation）✅ MERGED #85 `cf1794e9` + Cloudflare Pages deploy success**＝**FACTOR-ADD 前端接線全收尾**；**下一步＝回 JS→TS Stage 7 strict（或 owner 排序）+ owner prod 無痕驗收**。#82 SEC-REFRESH ✅ MERGED `452b478` / #83 OD-3 enforcement ✅ MERGED `a01d9571`+deploy）
...
- ✅ **FACTOR-ADD 前端 elevation 接線 Stage 1（TOTP/password）MERGED #84 `285b8987`（2026-06-15）+ Cloudflare Pages deploy success**：
  - **根因（已確診 2026-06-15，非 OD-3）**：owner 解綁 Discord 後重綁失敗 toast `Factor-add elevation required` ＝ `FACTOR_ADD_GRANT_REQUIRED`。#78（PR-A3 `7ae5558`）對 `oauth/[provider]/init.ts`（`is_binding`）／`webauthn/register-verify.ts`／`wallet/verify.ts` 三入口加 `requireFactorAddGrant`（需 `X-Factor-Add-Grant` header），但 SEC-FACTOR-ADD-A §11 staged PR 從頭沒排「前端接線」顆 → `src/js/dashboard.ts` 三 caller 裸打、不鑄 grant、無 `#elev_exchange` handler → passkey/wallet/OAuth-binding 自 #78 prod 全 403（登入/移除既有 factor 不受影響）。fail-closed、無安全破口、P1 可用性 regression。全綠 PR 仍出壞 feature 的盲點＝`tests/integration/_helpers.ts:383 seedFactorAddGrant` 直 INSERT grant 繞過前端、無 dashboard E2E。
  - **Stage 1 落地（純前端、後端零改、無 migration）**：`obtainFactorAddGrant(action)` + dedicated `openElevationModal`（OD-1）+ 三 caller 帶 `X-Factor-Add-Grant` header + `checkBindResult` ERR_KEY（elevation_required/consumed）+ i18n（dashboard.json 9 modal key×4 lang + api.ts API_ERROR_I18N 8 code×4 lang）+ `tests/dashboard-factor-add-wiring.test.ts`（6 case，node:vm 載 built bundle、無 jsdom）。**Dual Gate v3 全閉環**：plan-self-review → ChatGPT Arch r1 REVISE〔C1 transport／C2 wiring〕→ r2 APPROVED → Codex Plan CODING_ALLOWED → code-self-review 3 fix → Codex Code CODE_APPROVED → ChatGPT FAITHFULNESS_APPROVED；CI 全綠（lint/ratchet898/cov90.28%/test:int1328/build/audit）。plan `docs/audit/factor-add-frontend-wiring-plan.md`。**owner 有 TOTP → 可重綁 Discord＝原始症狀解**；OAuth-only 帳號 Stage 1 僅引導文案（功能於 Stage 2 補）。
- ✅ **FACTOR-ADD Stage 2（OAuth-only OAuth-reauth elevation）MERGED #85 `cf1794e9`（2026-06-15）+ Cloudflare Pages deploy success**：
  - **範圍**：OAuth-only 帳號（無 TOTP 無密碼）的 factor-add elevation。`init?purpose=elevation&action=<action>` roundtrip（既綁 provider OAuth-reauth）→ callback 5a 驗 (provider,provider_id) match + OD-3 flagged 不鑄 → 一次性 `#elev_exchange=<code>`（2min、session 綁、fragment 交付）→ dashboard handler → `POST /api/auth/elevation/exchange`（**skipRefresh:true**）→ 續原 factor-add ceremony（add_passkey/bind_wallet/bind_identity，explicit preset grant）。pending-action（action + bind_identity target provider）跨 redirect 走 sessionStorage（非敏感路由 context；grant/code 不入 storage）。**後端零改、無 migration**（primitive 在 #77 `eb7e018`/#78 `7ae5558` 已建全）。
  - **落地**：`startOAuthReauthElevation` + dedicated `openReauthElevationModal`（候選 ∩ BIND_PROVIDERS、provider `encodeURIComponent`）+ `checkElevExchange`（即剝 fragment、sync read-clear、defer 避 load-time TDZ）+ 三 caller optional `presetGrant`（無 shared mutable state）+ `checkBindResult` elev_error 全集 + `isFactorAddActionClient`/BIND_PROVIDERS guard + api.ts 3 code×4 lang + dashboard.json 13 key×4 lang（含 ko）+ ambient `__reauthProviders`。test `tests/dashboard-factor-add-stage2.test.ts` 22 case（node:vm built bundle、無 jsdom）。
  - **Dual Gate v3 全閉環**：plan-self-review → ChatGPT Arch APPROVED〔A1–A5 hardening〕→ Codex Plan CODING_ALLOWED〔V1–V11〕→ code-self-review〔1 fix **RACE-3** settled-after-await + O7 regression，pre-fix RED 已驗〕→ Codex Code CODE_APPROVED → ChatGPT Faithfulness APPROVED。plan `docs/audit/factor-add-elevation-stage2-plan.md`（§12–§17 全軌跡）。**main CI 全綠**（lint/ratchet898/cov90.28%/test:int1328/bundle/audit）+ **Cloudflare deploy success**；cache-bust dashboard.html `?v=96d2b662`。
  - **下一步**：owner prod 無痕驗收（OAuth-only 帳號用既綁 provider reauth 後新增 passkey/wallet/OAuth-identity）；**回 JS→TS Stage 7 strict（或 owner 排序）**。
