...
**更新：2026-06-15**（**FACTOR-ADD Stage 1（TOTP/password elevation）✅ MERGED #84 `285b8987` + Cloudflare Pages deploy success**；**Stage 2（OAuth-only OAuth-reauth elevation）plan 進行中＝現行主線（插隊在回 Stage 7 strict 之前）**——接手讀本塊 + `docs/audit/factor-add-elevation-stage2-plan.md`。#82 SEC-REFRESH ✅ MERGED `452b478` / #83 OD-3 enforcement ✅ MERGED `a01d9571`+deploy）
...
- ⚠ **FACTOR-ADD Stage 2（OAuth-only OAuth-reauth elevation）— 現行主線（插隊在回 Stage 7 strict 之前）**：
  - **範圍**：OAuth-only 帳號（無 TOTP 無密碼）的 factor-add elevation。`init?purpose=elevation&action=<action>` roundtrip（用既有綁定 provider 做 OAuth-reauth）→ callback 5a 驗 (provider,provider_id) match 既綁 + OD-3 flagged 不鑄 → 一次性 `#elev_exchange=<code>`（2min、session 綁，fragment 交付）→ dashboard fragment handler → `POST /api/auth/elevation/exchange` 換 grant → 續原 factor-add ceremony（add_passkey/bind_wallet/bind_identity）。pending-action（action + bind_identity 的 target provider）跨 redirect 走 sessionStorage 持久化（非敏感路由 context；grant/code 不入 storage）。**後端零改、無 migration**（primitive 在 #77 `eb7e018`/#78 `7ae5558` 已建全、測全綠）。
  - **狀態**：branch `feat/factor-add-elevation-stage2`；plan `docs/audit/factor-add-elevation-stage2-plan.md`；走 Dual Gate v3（plan → dimension-A plan-self-review workflow → ChatGPT Arch Gate → Codex Plan Gate → code → Codex Code Gate → ChatGPT faithfulness → squash）。沿用 Stage 1 node:vm built-bundle test harness（無 jsdom）。**禁直推 main**。
