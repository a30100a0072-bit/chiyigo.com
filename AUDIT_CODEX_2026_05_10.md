...
| r8-other | device_uuid_prefix / credential_id_prefix / wallet address_prefix 也該套 hashIdentifierForAudit | ✅ Round 11 全套 |

## Round 11（Codex r9 audit follow-up，2026-05-10）

| # | 主張 | 修復 |
|---|------|------|
| r9-1 | hashIdentifierForAudit domain 應加 namespace 防多系統撞名 | ✅ `<domain>:v1` → `chiyigo.audit.<domain>:v1` |
| r9-2 | 命名 hmac16 vs guest_id_hash 不一致 | ✅ 統一 `<id>_hmac16` / `<id>_hmac32` |
| r9-4 | 7 個 prefix audit site 全套 keyed HMAC | ✅ device-alerts.js / devices/logout.js / webauthn 3 處 / wallet 2 處；對應 device-alerts integration test 更新 |
| r9-5 | refresh aud 仍由 body 決定（全景 #1 殘餘風險） | ✅ migration 0037 加 `refresh_tokens.issued_aud`；7 個 INSERT site 寫入；refresh.js 用 tokenRow.issued_aud 取代 body.aud；mismatch 寫 `auth.refresh.aud_mismatch` warn audit；舊 row NULL 退回 backward compat |
| r9-3 | salt fallback prod 強制失敗（deployment guard 配套） | ⏭ 留 followup（Cloudflare 設定 / health check） |
| r9-other | webhook payload_hash / ai_audit retention / admin_audit_log email | ⏭ retention 政策題，獨立議題 |
