...
(function () {
    ...
    function applyLangI(lang) {
        ...
        document.querySelectorAll('[data-i18n-ph]').forEach(el => { const k = el.dataset.i18nPh; if (k && typeof t[k] === 'string')
            el.placeholder = t[k]; });
        document.querySelectorAll('[data-i18n-aria]').forEach(el => { const k = el.dataset.i18nAria; if (k && typeof t[k] === 'string')
        ...
        if (cached) {
            ...
            if (badge)
                badge.textContent = fmt(t.total_label, { n: cached.total });
        }
    }
    ...
    async function load(page = 1, q = '') {
        ...
        const t = T();
        if (!token) {
            showError(t.err_login_required);
            return;
        }
        const loading = document.getElementById('loading');
        const content = document.getElementById('content');
        ...
        if (res.status === 401 || res.status === 403) {
            showError(t.err_perm);
            return;
        }
        if (!res.ok) {
            showError(fmt(t.err_http, { n: res.status }));
            return;
        }
        ...
        const badge = document.getElementById('total-badge');
        if (badge)
            badge.textContent = fmt(t.total_label, { n: data.total });
        currentPage = page;
        currentQ = q;
    }
    function renderTable(rows) {
        ...
        const t = T();
        if (!rows || !rows.length) {
            tbody.innerHTML = `<tr><td colspan="8" class="empty">${t.no_data}</td></tr>`;
            return;
        }
    ...
    function renderCards(rows) {
        ...
        const t = T();
        if (!rows || !rows.length) {
            container.innerHTML = `<p class="empty">${t.no_data}</p>`;
            return;
        }
    ...
    document.getElementById('modal')?.addEventListener('click', e => { if (e.target === e.currentTarget)
    ...
    async function openAuditCleanup() {
        ...
        const tok = getToken();
        if (!tok) {
            list.innerHTML = `<p class="empty-state empty-state--error">未登入</p>`;
            return;
        }
        ...
        if (!r || !r.ok) {
            list.innerHTML = `<p class="empty-state empty-state--error">載入失敗</p>`;
            return;
        }
        const j = await r.json();
        const rows = j.rows ?? [];
        if (!rows.length) {
            list.innerHTML = `<p class="empty-state">沒有刪除紀錄</p>`;
            return;
        }
    ...
    async function auditDelGo(auditId, btn) {
        ...
        if (!su || !su.ok) {
            btn.disabled = false;
            btn.textContent = '清除';
            window.notify.error('step-up 失敗');
            return;
        }
    ...
    document.addEventListener('click', e => {
        ...
        const delBtn = target?.closest('[data-audit-del]');
        if (delBtn)
            return auditDelGo(Number(delBtn.dataset.auditDel), delBtn);
        // 共用：data-modal-close
        const mc = target?.closest('[data-modal-close]');
        if (mc && mc.dataset.modalClose)
            document.getElementById(mc.dataset.modalClose)?.classList.remove('open');
        // 退款列表內按鈕
        const rfApprove = target?.closest('[data-rf-approve]');
        if (rfApprove)
            return openRefundDecide(Number(rfApprove.dataset.rfApprove), 'approve');
        const rfReject = target?.closest('[data-rf-reject]');
        if (rfReject)
            return openRefundDecide(Number(rfReject.dataset.rfReject), 'reject');
        // 詳情 modal 內：保存 / 刪除
        const ra = target?.closest('[data-ra-action]');
        if (ra)
            return openReqAction(ra.dataset.raAction, Number(ra.dataset.raId));
    });
    // 點 backdrop 自動關
    ...
    document.getElementById('rd-confirm-btn')?.addEventListener('click', async () => {
        ...
        const btn = document.getElementById('rd-confirm-btn');
        const modal = document.getElementById('modal-refund-decide');
        if (!otpEl || !noteEl || !btn || !modal)
            return;
        const otp = otpEl.value.trim();
        const note = noteEl.value.trim();
        if (!/^\d{6}$/.test(otp)) {
            setRdMsg('OTP 須為 6 位數字', 'err');
            return;
        }
        const tok = getToken();
        if (!tok) {
        ...
        const { step_up_token } = await su.json();
        if (!step_up_token) {
            setRdMsg('未拿到 step-up token', 'err');
            btn.disabled = false;
            return;
        }
        ...
        if (!r || !r.ok) {
            ...
            if (r) {
                try {
                    const j = await r.json();
                    msg = (j.error || msg) + (j.rtn_msg ? ` / ${j.rtn_msg}` : '');
                }
                catch { }
            }
            setRdMsg(msg, 'err');
    ...
    document.addEventListener('click', e => {
        ...
        if (!t)
            return;
        if (t.dataset.openModal)
            openModal(Number(t.dataset.openModal));
        else if (t.dataset.loadPage)
            load(Number(t.dataset.loadPage), currentQ);
    });
})();
