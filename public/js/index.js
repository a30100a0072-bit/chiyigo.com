...
(function () {
    ...
    function handleOverlayClick(e) {
        if (e.target === document.getElementById('m-overlay'))
            closeOverlay();
    }
    ...
    document.addEventListener('keydown', e => { if (e.key === 'Escape' && document.getElementById('m-overlay')?.classList.contains('is-open'))
        closeOverlay(); });
    (function () {
        const THRESHOLD = 110;
    ...
    // ── Scroll reveal ──
    const revealObs = new IntersectionObserver(es => es.forEach(e => { if (e.isIntersecting) {
        e.target.classList.add('revealed');
        revealObs.unobserve(e.target);
    } }), { threshold: 0.1 });
    document.querySelectorAll('[data-reveal]').forEach(el => revealObs.observe(el));
    // ── Sidebar active ──
    const sectionObs = new IntersectionObserver(es => es.forEach(e => {
        if (!e.isIntersecting)
            return;
    ...
    ['hero', 'services', 'process', 'cta'].forEach(id => { const el = document.getElementById(id); if (el)
    ...
    function initPts() { pts.length = 0; for (let i = 0; i < NODES; i++)
        pts.push({ x: Math.random() * cvs.width, y: Math.random() * cvs.height, vx: (Math.random() - .5) * SPEED * 2, vy: (Math.random() - .5) * SPEED * 2, r: Math.random() * 1.5 + 1 }); }
    function draw() {
        ...
        for (let i = 0; i < pts.length; i++)
            for (let j = i + 1; j < pts.length; j++) {
                ...
                if (d < MAXDIST) {
                    ...
                    ctx.strokeStyle = `rgba(${r},${g},${b},${lop * (1 - d / MAXDIST)})`;
                    ctx.lineWidth = 0.75;
                    ctx.stroke();
                }
            }
