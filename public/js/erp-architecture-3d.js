...
const NODE_COLOR = Object.fromEntries(['iam', 'crm', 'sales', 'finance', 'workflow', 'event', 'data', 'mdm', 'notify', 'file', 'integration', 'bi', 'ai', 'metadata', 'knowledge', 'sre']
...
function initScene() {
    ...
    const key = new THREE.DirectionalLight(0x8c91ff, 0.6);
    key.position.set(300, 400, 300);
    scene.add(key);
    const rim = new THREE.DirectionalLight(0x6c6ee5, 0.3);
...
function setChain(name) {
    ...
    for (const { line } of edgeLines)
        line.visible = !activeChain;
    // 重建 chain segments
    if (activeChain)
        buildChain(activeChain);
    else
        clearChain();
...
function onClick(e) {
    ...
    if (hits.length) {
        ...
        if (activeKind === ud.kind && activeId === ud.id)
            setActive(null, null);
        else
            setActive(ud.kind, ud.id);
    }
    else {
        setActive(null, null);
    }
...
AUTO_BTN?.addEventListener('click', () => {
    ...
    if (txt)
        txt.textContent = autoRotate ? (tDict().l3d_autorotate || 'Auto') : (tDict().l3d_paused || 'Paused');
});
...
function applyLang(lang) {
    ...
    buildA11yList();
    if (activeKind === 'layer')
        renderLayerPanel(activeId);
    else if (activeKind === 'node')
        renderNodePanel(activeId);
    if (AUTO_BTN) {
        const txt = AUTO_BTN.querySelector('[data-i18n]');
        if (txt)
            txt.textContent = autoRotate ? (t.l3d_autorotate || 'Auto') : (t.l3d_paused || 'Paused');
    }
    // chain note 翻譯
...
langDropdown?.addEventListener('click', e => { const opt = e.target?.closest('.lang-opt'); if (!opt)
    return; applyLang(opt.dataset.lang); langDropdown.classList.remove('open'); });
