...
(function () {
    ...
    document.getElementById('f-clear')?.addEventListener('click', () => {
        ['f-q', 'f-user-id', 'f-from', 'f-to'].forEach(id => { const el = document.getElementById(id); if (el)
            el.value = ''; });
  ...
  `;
    ...
    function reqLink(id) {
        if (!id)
            return `<span class="mono mono-dim">${esc(T().deleted_req || '已刪')}</span>`;
  ...
  `;
    ...
    async function loadAgg() {
        ...
        const tt = T();
        if (!buckets.length) {
            wrap.innerHTML = `<p class="txt-hint-dim">${esc(tt.agg_empty || '無資料')}</p>`;
            return;
        }
