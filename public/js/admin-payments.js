...
(function () {
    ...
    async function load() {
        ...
        try {
            ...
            if (err?.code === 'SESSION_EXPIRED')
                return; // apiFetch 已 redirect
            if (err?.status === 403)
                return showError(t.err_forbidden);
    ...
    function renderCards(rows) {
        ...
        if (!c)
            return;
        if (!rows.length) {
            c.innerHTML = `<div class="empty">${esc(t.empty_text)}</div>`;
            return;
        }
    ...
    document.addEventListener('click', e => {
        ...
        if (closer && closer.dataset.modalClose)
    ...
    function openDetail(id) {
        ...
        if (meta?.payment_info) {
            ...
            if (info.expire_date)
                lines += `<div class="info-line"><span class="k">${esc(t.detail_expire)}</span><span class="v">${esc(info.expire_date)}</span></div>`;
            if (lines)
                metaHtml += `<div class="modal-row"><span class="lbl">${esc(t.detail_payment_info)}</span><div class="val"><div class="modal-info-block">${lines}</div></div></div>`;
        }
    ...
    function openRefund(id) {
        ...
        const otpEl = document.getElementById('refund-otp');
        const reason = document.getElementById('refund-reason');
        if (!summary || !otpEl || !reason)
            return;
        summary.textContent =
    ...
    document.getElementById('refund-confirm-btn')?.addEventListener('click', async () => {
        ...
        if (!otpEl || !reasonEl || !btn)
            return;
        const otp = otpEl.value.trim();
        const reason = reasonEl.value.trim();
        if (!/^\d{6}$/.test(otp)) {
            setRefundMsg(t.refund_err_otp, 'err');
            return;
        }
        if (!refundIntentId) {
            setRefundMsg(t.refund_err_no_intent, 'err');
            return;
        }
        btn.disabled = true;
        ...
        if (!r) {
            setRefundMsg(t.err_network, 'err');
            btn.disabled = false;
            return;
        }
        if (!r.ok) {
    ...
    async function adminDelGo() {
        ...
        if (btn.dataset.armed !== '1') {
            ...
            if (_adminDelArmTimer)
                clearTimeout(_adminDelArmTimer);
        ...
        if (_adminDelArmTimer) {
            clearTimeout(_adminDelArmTimer);
            _adminDelArmTimer = null;
        }
        const otpEl = document.getElementById('admin-del-otp');
        const otp = otpEl?.value.trim() ?? '';
        if (!/^\d{6}$/.test(otp)) {
            setAdminDelMsg('請輸入 6 位數字 2FA', 'err');
            return;
        }
        btn.disabled = true;
        setAdminDelMsg('驗證 2FA…', '');
        ...
        try {
            ...
            if (!stepUpToken)
                throw new Error('missing step_up_token');
        }
        catch (e) {
            const err = e;
            if (err?.code === 'SESSION_EXPIRED')
    ...
    document.addEventListener('click', e => {
        const target = e.target;
        const a = target?.closest('[data-action]')?.dataset?.action;
        if (a === 'admin-del-cancel')
            return closeAdminDelete();
        if (target?.id === 'admin-del-go')
            return adminDelGo();
    });
    // ── Init ───────────────────────────────────────────────
    load();
