...
(function () {
    ...
    function applyLang(lang) {
        ...
        const cache = window._lastAiResult;
        if (cache)
            renderResult(cache);
    }
    ...
    document.getElementById('m-top-lang-drop')?.addEventListener('click', e => { const opt = e.target?.closest('.lang-opt'); if (!opt)
        return; applyLang(opt.dataset.lang); document.getElementById('m-top-lang-drop')?.classList.remove('open'); });
    applyLang(curLang);
    // ── Reveal animation ──────────────────────────────
    ...
    function getFingerprint() {
        try {
            ...
            if (!ctx)
                return fpHash(navigator.userAgent + '|' + navigator.language);
    ...
    if (window.turnstile && TURNSTILE_SITEKEY)
        renderTurnstile();
    ...
    function renderResult(r) {
        ...
        if (rService)
            rService.textContent = labelMap('service', r.service_type);
        if (rBudget)
            rBudget.textContent = labelMap('budget', r.budget);
        if (rTimeline)
            rTimeline.textContent = labelMap('timeline', r.timeline);
        if (rSummary)
            rSummary.textContent = r.summary;
    }
    btnAnal?.addEventListener('click', async () => {
        if (!errEl || !inputEl || !cardRes)
            return;
        clearErr(errEl);
        ...
        if (prompt.length > 500) {
            showErr(errEl, 'err_too_long');
            return;
        }
        btnAnal.classList.add('is-loading');
        ...
        try {
            ...
            if (res.status === 401) {
                showErr(errEl, 'err_auth');
                setTimeout(() => location.replace('/login.html'), 1200);
                return;
            }
            if (res.status === 429) {
                showErr(errEl, 'err_rate_limit');
                return;
            }
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                if (data.code === 'TOO_LONG')
                    showErr(errEl, 'err_too_long');
                else if (data.code === 'BLOCKED')
                    showErr(errEl, 'err_blocked');
                else if (data.code === 'AI_ERROR')
                    showErr(errEl, 'err_ai');
                else if (data.code === 'INVALID_OUTPUT')
                    showErr(errEl, 'err_ai');
                else
                    showErr(errEl, data.error || 'err_network');
                return;
            }
            _lastResult = data;
            renderResult(data);
            ...
            btnAnal.classList.remove('is-loading');
    ...
    btnConf?.addEventListener('click', async () => {
        ...
        if (!me?.email) {
            showErr(errEl2, 'err_auth');
            return;
        }
        btnConf.classList.add('is-loading');
        ...
        try {
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                showErr(errEl2, data.error || 'err_submit');
                return;
            }
            // 成功
            cardRes.classList.remove('show');
            ...
            if (succRef && data.id)
                succRef.textContent = '// ref: #' + data.id;
            ...
            btnConf.classList.remove('is-loading');
