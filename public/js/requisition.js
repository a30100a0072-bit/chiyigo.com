...
(function () {
    ...
    function setLoading(on) {
        if (!submitBtn || !btnText || !btnLoad || !btnIcon)
            return;
    ...
    function showErr(msg) { if (formError) {
        formError.textContent = msg;
        formError.classList.add('visible');
    } }
    function clearErr() { if (formError) {
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (_token)
                fetchHeaders['Authorization'] = 'Bearer ' + _token;
    ...
    function applyLangI(lang) {
        ...
        document.querySelectorAll('[data-i18n]').forEach(el => { const k = el.dataset.i18n; if (k && t[k] !== undefined)
            el.textContent = t[k]; });
        document.querySelectorAll('[data-i18n-ph]').forEach(el => { const k = el.dataset.i18nPh; if (k && t[k] !== undefined)
            el.placeholder = t[k]; });
