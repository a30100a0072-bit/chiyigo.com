...
(function () {
    ...
    document.addEventListener('click', e => {
        ...
        const ap = target?.closest('[data-rf-approve]');
        if (ap)
            return openDecide(Number(ap.dataset.rfApprove), 'approve');
        const rj = target?.closest('[data-rf-reject]');
        if (rj)
            return openDecide(Number(rj.dataset.rfReject), 'reject');
        const close = target?.closest('[data-modal-close]');
        if (close && close.dataset.modalClose)
            document.getElementById(close.dataset.modalClose)?.classList.remove('open');
    });
    ...
    document.getElementById('rd-confirm-btn')?.addEventListener('click', async () => {
        ...
        const t = T();
        if (!/^\d{6}$/.test(otp)) {
            setMsg(t.msg_otp_invalid || 'OTP 須為 6 位數字', 'err');
            return;
        }
        btn.disabled = true;
        ...
        try {
            ...
            if (!step_up_token) {
                setMsg(t.msg_step_up_no_token || '未拿到 step-up token', 'err');
                btn.disabled = false;
                return;
            }
        }
