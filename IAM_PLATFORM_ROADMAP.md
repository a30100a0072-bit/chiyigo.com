...
| 日期 | 事件 |
|---|---|
| 2026-05-16 | **JS→TS 漸進遷移 Stage 2/3 累計**：infra 治理層，不改任何 phase scope。Stage 0 ratchet + Stage 1 PR 1.0/1.1 + Stage 2（PR 1-7 後段 auth/payments/audit chain + chain PR-A/B'/C utils rename）+ Stage 3 PR-1（`functions/ai/assist`）+ PR-2（`functions/api/auth/webauthn/register-verify`，HEAD `f316dad`）。ratchet baseline 進展 1097/150 → 983/191（-114 errors / +41 cleanFiles），每 PR 走 codex chain（auth/payments/audit 熱區）；紀律：rename + 最小 inline TS、importer extensionless、reduce-error PR 收尾 `typecheck:baseline:update`、auth/payments first-do-no-harm（0 行為變更）。所有 PR 全 typecheck:ratchet / build:functions / 對應 integration tests 綠 + codex r1 LGTM。詳見 memory `project_js_to_ts_migration.md`。**Stage 3 下一發候選**：`payments/intents.js` 4 / 4 個 1-error 金流小檔 / `payment-vendors/ecpay.js` 12（金流熱區獨立 codex chain）|
| 2026-05-16 | **F-3 Phase 2 audit cold archive PR 3.1 debug aggregate post-deploy watch PASS**：2026-05-14 首跑 cron + 2 天觀察期，11 emitter delta=0 / `audit.aggregate.debug.run_skipped` 配 `no_rows_eligible` 正常觸發 / agg_debug_rows=0 / debug_failure_total=15 / 0 dictionary 違反；cron drift +4.5h 為 GitHub Actions best-effort 非故障。下一步順序（5 輪 codex 設計 alignment）：0.2c-pre-1 lock-compat write path → 0.2c-pre-2 force_purge 423 catch → preview gate → prod lock → PR 2.1c endgame discard → PR 4a live flip + canary 2 週 + 5 query/coverage gate → PR 4b cron purge（CRON_PURGE_ENABLED flag）。F-2 manifest.severities prod 驗收等流量自然帶 |
| 2026-05-16 | **F-2 codex 金流 review r1 chain 24hr post-deploy watch PASS**：2026-05-13/14 共 11 輪 codex 24 commits + 2 migrations 上 prod `b5e4aaf`；341 unit + 587 integration 全綠；24hr 監測 6 event_type 全 0 hit / DLQ 4 unreplayed 全是預先存在 delta=0。衍生 4 memory：DLQ strict throw、updateStatus structured outcome、codex review iteration、Pages deploy + D1 migration sync。**F-2 金流完整 smoke 驗證延後到全部金流寫完**（user 2026-05-14 拍板）：等 F-2 真未做項（withdraw / Stripe / TapPay / 訂閱 / 反詐欺）寫完一次性 smoke，覆蓋 11 個 debug_failure emitter 路徑 + CAS 競爭 + refund vendor 掛掉等 edge case |
| 2026-05-14 | **F-3 Phase 2 audit cold archive PR 3.3 part 3 完工 + codex 9 輪 LGTM**（HEAD `a8977f7`）：F-2 manifest.severities 補欄位 + AGGREGATE_DEBUG 觀測位點完整鋪設；歷史 raw row 由 live pipeline 自然帶（不 backfill）。架構原則新增「prod 永不 unlock→DELETE→relock 演練」（R2 lock 不只擋 DELETE 也擋同 key PUT；same-key state machine 上 lock 前必改 write-once key + putWithRetry lock-aware）。衍生 memory：`feedback_r2_lock_overwrite_design` / `feedback_r2_lock_propagation_canary`（lock/lifecycle propagation 需 canary） |
| 2026-05-12 | **isWebClient 規格 B**（PR #1 squash `50fb106`）：抽 `functions/utils/cookies.js#isWebClient(request, { platform })` 共用 helper，4 個雙通道 endpoint（local/login、local/register、webauthn/login-verify、2fa/verify）+ oauth/token 全切到共用版。修 P2 bug：舊版 `!device_uuid && (!platform || platform==='web')` 把裝置綁定概念誤綁進通道判斷 — web client 不小心送 `device_uuid` → `refresh_token` 寫進 body 違反 HttpOnly；App 沒送 platform 又沒 device_uuid → 嘗試 Set-Cookie 靜默失敗。改規格 B：Origin 為 source of truth（`chiyigo.com` / `*.chiyigo.com`），explicit non-web `platform`（ios/android/...）→ 永遠 non-web（含 hybrid webview 在 chiyigo origin 開但宣告 ios），`device_uuid` 完全不參與。跨站 Origin + 偽造 `platform=web` → non-web（P1 鎖）。**Scope 例外**：`oauth/[provider]/callback.js` + `oauth/bind-email.js` 仍無條件 Set-Cookie 不納入（browser-only top-level navigation 觸發）；`refresh.js` 留 `!!cookieToken`（cookie 存在本身就證明 web）。測試：`tests/cookies.test.js` 11 unit cases（含 P1 鎖 evil+platform=web）；5 個 integration 檔加 channel matrix，2FA / WebAuthn / OAuth code 都每 case fresh seed 避免 used_totp replay / challenge consume / DELETE...RETURNING 互打。276 unit + 506 integration 全綠。Codex 三輪 review approve 後 squash merge |
| 2026-05-07 | Dashboard device sessions UX 收尾（commits `7320e77` cache bump / `157f948` self-logout + web-XXX label / `81f55b6` HAVING active_count > 0 + revoked check / `9b86faf` isWeb ReferenceError）：(1) bump `auth-ui.js?v=15` / `api.js?v=2` / `dashboard.js?v=devid3` 強制重抓避開瀏覽器舊版 cache；(2) `renderDevices` 把 `web-` 前綴當瀏覽器 session 顯示「瀏覽器 Session · XXXXXXXX」（slice(4,12) 8 碼），mobile App 才走「App 裝置」分支；(3) `logoutDevice` 比對 `localStorage.chiyigo.device_uuid === deviceUuidAttr` 視作「自己這台」清 sessionStorage + 跳 login，否則只 `loadDevices()` 重載；(4) `GET /api/auth/devices` 加 `HAVING active_count > 0` 把全 revoked group 直接濾掉；(5) 前端檢查 `{revoked}` response — `revoked === 0` 顯示「此裝置已無有效 session」+ 重載而非 success toast。不引入 per-session jti / token version bump（access_token 15min 短效灰色期可接受，jti 對自登 UX 殺雞牛刀）。codex 驗收 4/4 PASS：0 使用中不再出現、web-XXXX label 正確、自登跳 login、別台只消失自己留 |
| 2026-05-07 | OAuth device_uuid propagation（commit `d2e5e64`）：補上一條 commit `431eea7` 漏掉的 OAuth flow 路徑 — client JS 在按 OAuth 按鈕前寫 `chiyigo_oauth_device` cookie（10min SameSite=Lax Secure）；`functions/api/auth/oauth/[provider]/callback.js` 用 `readOAuthDeviceCookie()` 讀出後寫進 refresh_tokens.device_uuid，同時 `CLEAR_OAUTH_DEVICE_COOKIE` 清掉 cookie；alert 也改傳 webDeviceUuid 讓 OAuth 換瀏覽器也能觸發新裝置 email。沒 cookie（舊 client / mobile App）仍寫 NULL backwards compat。callback.test.js 7/7 PASS |
| 2026-05-07 | Browser-level device identity（commit `431eea7`）：每瀏覽器一次性 `web-<uuid>` 存 `localStorage['chiyigo.device_uuid']`；登入時帶 `body.device_uuid`、refresh 時帶 `X-Device-Id` header → 桌面 / 手機 / 不同 browser 在 dashboard「我的裝置」獨立顯示、可分別登出。後端 schema / API contract 完全不改（refresh.js 已支援 X-Device-Id）；既有 NULL row session 走 `tokenRow.device_uuid !== null` 跳過比對，**無需重登**。Frontend 4 處 inline `_chiyigoGetDeviceUuid()`（auth-ui.js / api.js / sidebar-auth.js / src/js/ai-assistant.js — 不是 ESM 不能 import；改 logic 要同步 4 處）。Defensive：localStorage 不可用退 in-memory；crypto.randomUUID 不可用回 null + console.warn 不阻擋登入。tests/device-id.test.js 7 條 unit + login/refresh integration 18/18 全 PASS |
| 2026-05-07 | Turnstile UX 優化（commit `6790cfc`）：CF dashboard sitekey 切 Non-Interactive（不動 Invisible 保 security presence）；前端三件 — `.ts-wrap` 預留 65px min-height 防 CLS / `.ts-hint` 4 語系 hint「資安驗證準備中，可先填寫帳號與密碼」/ widget `data-callback="onTurnstileReady"` 簽完 token 自動隱藏 hint。不動 server verify、不動 auth flow 順序、不降 CSP（無新 inline）。手機慢網路體感明顯改善；inputs 全程不 disable。LOG-ONLY config：mode 是 sitekey-level CF dashboard 設定，code 端只在 widget 上方留 comment 標註此規約 |
...
| 2026-05-05 | Phase F-2 wave 5 admin-payments UI 頁（commit `aeaa277`）：src/pages/admin-payments.html — 後台金流對帳頁（sidebar 加「金流對帳」與 admin-requisitions 互通；filter bar status/vendor/user_id/date；totals row 顯示符合筆數+各 status count+成功總金額；table+pagination+detail modal+refund modal w/ OTP）；admin-payments.js — list/filter/pagination/detail modal + refund flow（先打 /api/auth/step-up 取 step_up_token → 打 /admin/payments/intents/:id/refund）；admin-payments.json 4 語；admin-requisitions.css 附加 .pay-badge/.filter-bar/.totals-row/.pay-action-btn/.modal-info-block/.refund-modal-body 共用樣式；refund.js 改用 effectiveScopesFromJwt（step-up token scope 純 elevated:* 不帶 admin:*，靠 role fallback 補 admin:payments；player role step-up 即使拿到 elevated:payment 也會 403）；admin-payments.test.js token shape 改成真實 step-up（純 elevated:payment + role=admin）；108 unit + 442 int 全綠 |
| 2026-05-05 | Phase F-2 wave 4 ECPay ATM/CVS info + admin 對帳 + 退款（commit `11f3e38`）：parseWebhook 加 ATM/CVS/Barcode 取號處理（RtnCode=2 + BankCode/vAccount → ATM；RtnCode=10100073 + PaymentNo → CVS；+ Barcode1/2/3 → barcode；status=processing；payment_info 抽出 method/v_account/payment_no/barcode_*/expire_date）；webhook handler [vendor].js mergeMetadata() 把 payment_info read-modify-write JSON merge 進 payment_intents.metadata；scopes.js 加 ADMIN_PAYMENTS = 'admin:payments'（admin/developer 自動繼承）；GET /api/admin/payments/intents — admin 對帳 list（filter user_id/status/vendor/from/to + paging + totals.count_by_status + totals.sum_subunit_succeeded）；POST /api/admin/payments/intents/:id/refund — 雙重 gate（access_token admin:payments + step-up elevated:payment + for_action='refund_payment'）；ecpayRefund() helper（CheckMacValue + form POST 到 /CreditDetail/DoAction Action=R + parse RtnCode）；TradeNo 從 payment_webhook_events.event_id 撈成功那筆（SUCCEEDED 分支 event_id 純 TradeNo 無 \_RtnCode 後綴）；ATM/CVS 退款不支援（ECPay refund API 僅信用卡）；dashboard renderPayments 加 amber info block 顯示 V 帳號 / CVS 代碼 / 條碼（status=processing 時）+ 5 個 i18n key × 4 語；新增 admin-payments.test.js（11 條：list 4 + refund 7）+ payments-ecpay.test.js 加 4 條 ATM/CVS/Barcode 解析 + e2e ATM metadata merge；108 unit + 442 int 全綠 |
| 2026-05-05 | Phase F-2 wave 3 dashboard 充值 UI + requisition 串接（commit `0bd6908`）：dashboard.html 加 payments-section（list + 充值 form）+ sidebar/overlay nav nav_payments；dashboard.js loadPayments / renderPayments / openPaymentForm / submitPaymentCheckout / redirectToEcpay（DOM API 動態建 hidden form CSP-safe 沒 inline script）；接 ?req=N query 自動展開表單 + 預填 requisition_id；新增 src/pages/payment-result.html（ECPay ClientBackURL 跳回頁，meta refresh 5s 回 dashboard）；requisition.html/js/css/i18n 4 改：success state 給登入 user 顯示「立即付款 →」連結帶 ?req=N 跳 dashboard；dashboard.json 31 個新 key × 4 語；public/_headers CSP form-action 加白名單 `https://payment-stage.ecpay.com.tw https://payment.ecpay.com.tw`（不加會被 CSP 擋掉跨站 form POST）；故意設計：requisition 串接是 metadata convention（提單時還沒議價不建 intent，由 dashboard 接收 query 預填欄位後存進 metadata.requisition_id）；108 unit + 427 int 全綠 |
